...
#ifndef FOLLY_EXT_UTF8_STRING_PIECE_H_
#define FOLLY_EXT_UTF8_STRING_PIECE_H_
...
public:
  ...
  /* implicit */ Utf8StringPiece(const fbstring& str)
  : b_(str.data()), e_(str.data() + str.size()) {}

  Utf8StringPiece(const fbstring& str, size_t startFrom) {
    if (UNLIKELY(startFrom > str.size())) {
      throw std::out_of_range("index out of range");
    }
    b_ = str.data() + startFrom;
  ...
  Utf8StringPiece(const fbstring& str,
                  size_t startFrom,
                  size_t size) {
    if (UNLIKELY(startFrom > str.size())) {
      throw std::out_of_range("index out of range");
...
#endif /* FOLLY_EXT_UTF8_STRING_PIECE_H_ */
