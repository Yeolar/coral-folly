...
char32_t utf8ToCodePoint(const unsigned char*& p,
  ...
  for (unsigned int i = 1; i != 3 && p + i < e; ++i) {
    ...
    if ((tmp & 0xC0) != 0x80) {
      if (skipOnError) return skip();
      throw std::runtime_error(
        to<std::string>("folly::utf8ToCodePoint i=", i,
                        " tmp=", (uint32_t)tmp));
    }
...
std::string toLower(const folly::StringPiece& sp) {
  std::string result;
...
std::string toUpper(const folly::StringPiece& sp) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
...
std::string toOppositeCase(const folly::StringPiece& sp) {
  std::string result;
...
std::string normalize(const folly::StringPiece& sp, NormalizationMode mode) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
...
std::string asciify(const folly::StringPiece& sp) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
  icu::UnicodeString us = icu::UnicodeString::fromUTF8(usp);
  UErrorCode err = U_ZERO_ERROR;
