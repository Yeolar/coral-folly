...
class SocketAddress {
 public:
  ...
  /**
   * Initialize this coral::SocketAddress from a struct sockaddr.
   *
   * Raises std::system_error on error.
   *
  ...
  const coral::IPAddress& getIPAddress() const;
...
template<>
struct hash<coral::SocketAddress> {
  size_t operator()(
