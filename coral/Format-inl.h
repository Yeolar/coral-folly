 ...
 *
 * Just like coral::detail::uintToBuffer in Conv.h, writes at the *end* of
...
void BaseFormatter<Derived, containerMode, Args...>::operator()(Output& out)
  ...
  auto outputString = [&out] (StringPiece s) {
    ...
    while (p != end) {
      ...
      if (p == end || *p != '}') {
        throw BadFormatArg("coral::format: single '}' in format string");
      }
      ++p;
    }
  };
  ...
  while (p != end) {
    ...
    if (p == end) {
      throw BadFormatArg("coral::format: '}' at end of format string");
    }
    ...
    if (q == nullptr) {
      throw BadFormatArg("coral::format: missing ending '}'");
    }
    ...
    if (hasDefaultArgIndex && hasExplicitArgIndex) {
      throw BadFormatArg(
          "coral::format: may not have both default and explicit arg indexes");
    }
...
void formatString(StringPiece val, FormatArg& arg, FormatCallback& cb) {
  if (arg.width != FormatArg::kDefaultWidth && arg.width < 0) {
    throw BadFormatArg("coral::format: invalid width");
  }
  if (arg.precision != FormatArg::kDefaultPrecision && arg.precision < 0) {
    throw BadFormatArg("coral::format: invalid precision");
  }
 ...
 * compatible with coral::toAppend and coral::to.
 */
