...
//
// In your .cpp file:
// namespace { coral::Singleton<MyExpensiveService> the_singleton; }
//
...
// struct Tag1 {};
// struct Tag2 {};
// coral::Singleton<MyExpensiveService> s_default;
// coral::Singleton<MyExpensiveService, Tag1> s1;
// coral::Singleton<MyExpensiveService, Tag2> s2;
// }
// ...
...
//
// namespace { coral::Singleton<MyExpensiveService> the_singleton(create,
//                                                                destroy); }
//
...
#ifndef CORAL_SINGLETON_TRY_GET
#define CORAL_SINGLETON_TRY_GET
#endif
...
class TypeDescriptorHasher {
 public:
  size_t operator()(const TypeDescriptor& ti) const {
    return coral::hash::hash_combine(ti.ti_, ti.tag_ti_);
  }
};
...
struct SingletonHolder : public SingletonHolderBase {
 ...
 private:
  ...
  std::shared_ptr<coral::Baton<>> destroy_baton_;
...
class SingletonVault {
 ...
 private:
  ...
  // coral::Singleton. The desruction will happen in the following order:
  // 1. Singletons, not managed by coral::Singleton, which were created after
  //    any of the singletons managed by coral::Singleton was requested.
  // 2. All singletons managed by coral::Singleton
  // 3. Singletons, not managed by coral::Singleton, which were created before
  //    any of the singletons managed by coral::Singleton was requested.
  ...
  mutable coral::RWSpinLock mutex_;
...
class Singleton {
 public:
  ...
  static std::shared_ptr<T> try_get() {
    ...
    if (!ret) {
      LOG(DFATAL) <<
        "coral::Singleton<" << getEntry().type().name() <<
