...
#ifndef CORAL_BASE_TRAITS_H_
#define CORAL_BASE_TRAITS_H_
...
#ifdef CORAL_HAVE_BITS_CXXCONFIG_H
 ...
 * namespace coral and make sure you keep the specialization of
...
#define CORAL_HAS_TRUE_XXX(name)                          \
...
CORAL_HAS_TRUE_XXX(IsRelocatable)
CORAL_HAS_TRUE_XXX(IsZeroInitializable)
CORAL_HAS_TRUE_XXX(IsTriviallyCopyable)

#undef CORAL_HAS_TRUE_XXX
}
...
/**
 * Use this macro ONLY inside namespace coral. When using it with a
 * regular type, use it like this:
 *
 * // Make sure you're at namespace ::coral scope
 * template<> CORAL_ASSUME_RELOCATABLE(MyType)
 *
 * When using it with a template type, use it like this:
 *
 * // Make sure you're at namespace ::coral scope
 * template<class T1, class T2>
 * CORAL_ASSUME_RELOCATABLE(MyType<T1, T2>)
 */
#define CORAL_ASSUME_RELOCATABLE(...) \
 ...
 * template<> CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(MyType)
 *
 * When using it with a template type, use it like this:
 *
 * // Make sure you're at namespace ::boost scope
 * template<class T1, class T2>
 * CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(MyType<T1, T2>)
 */
#define CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(...) \
  struct has_nothrow_constructor<  __VA_ARGS__ > : ::boost::true_type {};

/**
 * The CORAL_ASSUME_FBVECTOR_COMPATIBLE* macros below encode two
 ...
 *
 * Use CORAL_ASSUME_FBVECTOR_COMPATIBLE with regular types like this:
 *
 * CORAL_ASSUME_FBVECTOR_COMPATIBLE(MyType)
 *
 * The versions CORAL_ASSUME_FBVECTOR_COMPATIBLE_1, _2, _3, and _4
 ...
 * CORAL_ASSUME_FBVECTOR_COMPATIBLE_2(MyType)
 */

// Use this macro ONLY at global level (no namespace)
#define CORAL_ASSUME_FBVECTOR_COMPATIBLE(...)                           \
  namespace coral { template<> CORAL_ASSUME_RELOCATABLE(__VA_ARGS__) }   \
  namespace boost { \
  template<> CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(__VA_ARGS__) }
// Use this macro ONLY at global level (no namespace)
#define CORAL_ASSUME_FBVECTOR_COMPATIBLE_1(...)                         \
  namespace coral {                                                     \
  template <class T1> CORAL_ASSUME_RELOCATABLE(__VA_ARGS__<T1>) }       \
    namespace boost {                                                   \
    template <class T1> CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(__VA_ARGS__<T1>) }
// Use this macro ONLY at global level (no namespace)
#define CORAL_ASSUME_FBVECTOR_COMPATIBLE_2(...)                 \
  namespace coral {                                             \
  template <class T1, class T2>                                 \
  CORAL_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2>) }               \
    namespace boost {                                           \
    template <class T1, class T2>                               \
    CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(__VA_ARGS__<T1, T2>) }
// Use this macro ONLY at global level (no namespace)
#define CORAL_ASSUME_FBVECTOR_COMPATIBLE_3(...)                         \
  namespace coral {                                                     \
  template <class T1, class T2, class T3>                               \
  CORAL_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2, T3>) }                   \
    namespace boost {                                                   \
    template <class T1, class T2, class T3>                             \
    CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(__VA_ARGS__<T1, T2, T3>) }
// Use this macro ONLY at global level (no namespace)
#define CORAL_ASSUME_FBVECTOR_COMPATIBLE_4(...)                         \
  namespace coral {                                                     \
  template <class T1, class T2, class T3, class T4>                     \
  CORAL_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2, T3, T4>) }               \
    namespace boost {                                                   \
    template <class T1, class T2, class T3, class T4>                   \
    CORAL_ASSUME_HAS_NOTHROW_CONSTRUCTOR(__VA_ARGS__<T1, T2, T3, T4>) }

/**
 * Instantiate CORAL_ASSUME_FBVECTOR_COMPATIBLE for a few types. It is
...
// coral::to integral specializations can end up generating code
...
constexpr bool is_negative(T x) {
  return coral::detail::is_negative_impl<T, std::is_signed<T>::value>::check(x);
}

// same as `x <= 0`
template <typename T>
constexpr bool is_non_positive(T x) { return !x || coral::is_negative(x); }
...
CORAL_ASSUME_FBVECTOR_COMPATIBLE_3(std::basic_string);
CORAL_ASSUME_FBVECTOR_COMPATIBLE_2(std::vector);
CORAL_ASSUME_FBVECTOR_COMPATIBLE_2(std::list);
CORAL_ASSUME_FBVECTOR_COMPATIBLE_2(std::deque);
CORAL_ASSUME_FBVECTOR_COMPATIBLE_2(std::unique_ptr);
CORAL_ASSUME_FBVECTOR_COMPATIBLE_1(std::shared_ptr);
CORAL_ASSUME_FBVECTOR_COMPATIBLE_1(std::function);

// Boost
CORAL_ASSUME_FBVECTOR_COMPATIBLE_1(boost::shared_ptr);

#define CORAL_CREATE_HAS_MEMBER_TYPE_TRAITS(classname, type_name) \
  template <typename T> \
  struct classname { \
    template <typename C> \
...
#define CORAL_CREATE_HAS_MEMBER_FN_TRAITS_IMPL(classname, func_name, cv_qual) \
...
/*
 * The CORAL_CREATE_HAS_MEMBER_FN_TRAITS is used to create traits
 ...
 * namespace {
 *
 * CORAL_CREATE_HAS_MEMBER_FN_TRAITS(has_test_traits, test);
 *
 * } // unnamed-namespace
 *
 * void some_func() {
 ...
 * @author: Marcelo Juchem <marcelo@fb.com>
 */
#define CORAL_CREATE_HAS_MEMBER_FN_TRAITS(classname, func_name) \
  template <typename, typename> class classname; \
  CORAL_CREATE_HAS_MEMBER_FN_TRAITS_IMPL(classname, func_name, ); \
  CORAL_CREATE_HAS_MEMBER_FN_TRAITS_IMPL(classname, func_name, const); \
  CORAL_CREATE_HAS_MEMBER_FN_TRAITS_IMPL( \
      classname, func_name, /* nolint */ volatile); \
  CORAL_CREATE_HAS_MEMBER_FN_TRAITS_IMPL( \
      classname, func_name, /* nolint */ volatile const)

#endif //CORAL_BASE_TRAITS_H_
