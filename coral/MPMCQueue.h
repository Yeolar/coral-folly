...
#include <coral/detail/TurnSequencer.h>
...
/// that don't use noexcept but that are marked coral::IsRelocatable
/// and boost::has_nothrow_constructor, which is common for coral types.
/// In particular, if you can declare CORAL_ASSUME_FBVECTOR_COMPATIBLE
...
class MPMCQueue : boost::noncopyable {
 ...
 private:
  ...
  size_t CORAL_ALIGN_TO_AVOID_FALSE_SHARING capacity_;
  ...
  Atom<uint64_t> CORAL_ALIGN_TO_AVOID_FALSE_SHARING pushTicket_;

  /// Dequeuers get tickets from here
  Atom<uint64_t> CORAL_ALIGN_TO_AVOID_FALSE_SHARING popTicket_;
  ...
  Atom<uint32_t> CORAL_ALIGN_TO_AVOID_FALSE_SHARING pushSpinCutoff_;

  /// The adaptive spin cutoff when the queue is empty on dequeue
  Atom<uint32_t> CORAL_ALIGN_TO_AVOID_FALSE_SHARING popSpinCutoff_;
...
struct SingleElementQueue {
  ...
  void dequeue(uint32_t turn,
    ...
    dequeueImpl(turn,
                ...
                updateSpinCutoff,
                elem,
                typename std::conditional<coral::IsRelocatable<T>::value,
                                          ImplByRelocation,
                                          ImplByMove>::type());
  }
