...
#ifndef CORAL_FBVECTOR_H
#define CORAL_FBVECTOR_H
...
#define CORAL_FBV_UNROLL_PTR(first, last, OP) do {  \
  ...
  struct Impl : public Allocator {
    ...
    void init(size_type n) {
      if (UNLIKELY(n == 0)) {
        ...
        size_type sz = coral::goodMallocSize(n * sizeof(T)) / sizeof(T);
        b_ = D_allocate(sz);
        e_ = b_;
        z_ = b_ + sz;
      }
...
private:
  ...
  static void S_destroy_range(T* first, T* last) noexcept {
    if (!boost::has_trivial_destructor<T>::value) {
      ...
      #define CORAL_FBV_OP(p) (p)->~T()
      CORAL_FBV_UNROLL_PTR(first, last, CORAL_FBV_OP)
      #undef CORAL_FBV_OP
    }
  }
  ...
  static void S_uninitialized_fill_n(T* dest, size_type n) {
    if (coral::IsZeroInitializable<T>::value) {
  ...
  void D_uninitialized_copy_a(T* dest, It first, It last) {
    if (usingStdAllocator::value) {
      if (coral::IsTriviallyCopyable<T>::value) {
...
private:
  ...
  // dispatch type trait
  typedef std::integral_constant<bool,
      coral::IsRelocatable<T>::value && usingStdAllocator::value
    > relocate_use_memcpy;
  ...
  void relocate_done(T* /*dest*/, T* first, T* last) noexcept {
    if (coral::IsRelocatable<T>::value && usingStdAllocator::value) {
...
public:
  ...
  void reserve(size_type n) {
    ...
    auto newCap = coral::goodMallocSize(n * sizeof(T)) / sizeof(T);
  ...
  void shrink_to_fit() noexcept {
    auto const newCapacityBytes = coral::goodMallocSize(size() * sizeof(T));
    ...
    if ((usingJEMalloc() && usingStdAllocator::value) &&
        newCapacityBytes >= coral::jemallocMinInPlaceExpandable &&
...
private:
  ...
  bool reserve_in_place(size_type n) {
    ...
    if ((impl_.z_ - impl_.b_) * sizeof(T) <
      coral::jemallocMinInPlaceExpandable) return false;

    auto const newCapacityBytes = coral::goodMallocSize(n * sizeof(T));
    void* p = impl_.b_;
...
private:
  ...
  size_type computePushBackCapacity() const {
    ...
    if (capacity() < coral::jemallocMinInPlaceExpandable / sizeof(T)) {
      return capacity() * 2;
    }
...
private: // we have the private section first because it defines some macros
  ...
  size_type computeInsertCapacity(size_type n) {
    ...
    size_type ac = coral::goodMallocSize(nc * sizeof(T)) / sizeof(T);
    return ac;
  }
  ...
  //  some macros: CORAL_FBVECTOR_INSERT_(START|TRY|END)
  ...
  #define CORAL_FBVECTOR_INSERT_START(cpos, n)                                \
    assert(isValid(cpos));                                                    \
  ...
  #define CORAL_FBVECTOR_INSERT_TRY(cpos, n)                                  \
    } catch (...) {                                                           \
      if (fresh) {                                                            \
  ...
  #define CORAL_FBVECTOR_INSERT_END(cpos, n)                                  \
        M_deallocate(b, newCap);                                              \
        throw;                                                                \
      }                                                                       \
...
public:
  ...
  iterator emplace(const_iterator cpos, Args&&... args) {
    CORAL_FBVECTOR_INSERT_START(cpos, 1)
      M_construct(start, std::forward<Args>(args)...);
    CORAL_FBVECTOR_INSERT_TRY(cpos, 1)
      M_destroy(start);
    CORAL_FBVECTOR_INSERT_END(cpos, 1)
  }
  ...
  iterator insert(const_iterator cpos, size_type n, VT value) {
    ...
    CORAL_FBVECTOR_INSERT_START(cpos, n)
      D_uninitialized_fill_n_a(start, n, value);
    CORAL_FBVECTOR_INSERT_TRY(cpos, n)
      D_destroy_range_a(start, start + n);
    CORAL_FBVECTOR_INSERT_END(cpos, n)
  }
...
void fbvector<T, Allocator>::emplace_back_aux(Args&&... args) {
  size_type byte_sz = coral::goodMallocSize(
    computePushBackCapacity() * sizeof(T));
  if (usingStdAllocator::value
      && usingJEMalloc()
      && ((impl_.z_ - impl_.b_) * sizeof(T) >=
          coral::jemallocMinInPlaceExpandable)) {
    ...
    size_type lower = coral::goodMallocSize(sizeof(T) + size() * sizeof(T));
