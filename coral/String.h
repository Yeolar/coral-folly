...
#ifndef CORAL_BASE_STRING_H_
#define CORAL_BASE_STRING_H_
...
#ifdef CORAL_HAVE_DEPRECATED_ASSOC
...
inline
std::string toStdString(const coral::fbstring& s) {
 ...
 */
std::string stringPrintf(CORAL_PRINTF_FORMAT const char* format, ...)
  CORAL_PRINTF_FORMAT_ATTR(1, 2);

/* Similar to stringPrintf, with different signature. */
void stringPrintf(std::string* out, CORAL_PRINTF_FORMAT const char* fmt, ...)
  CORAL_PRINTF_FORMAT_ATTR(2, 3);
 ...
 */
double prettyToDouble(coral::StringPiece *const prettyString,
                      const PrettyType type);

/*
 * Same as prettyToDouble(coral::StringPiece*, PrettyType), but
 * expects whole string to be correctly parseable. Throws std::range_error
 * otherwise
 */
double prettyToDouble(coral::StringPiece prettyString, const PrettyType type);
...
inline fbstring exceptionStr(const std::exception& e) {
  ...
  return coral::to<fbstring>(demangle(typeid(e)), ": ", e.what());
#else
  return coral::to<fbstring>("Exception (no RTTI available): ", e.what());
#endif
}
...
{
#ifdef CORAL_HAS_RTTI
  return coral::to<fbstring>(demangle(typeid(e)));
#else
 ...
 * Examples:
 *
 *   std::vector<coral::StringPiece> v;
 *   coral::split(":", "asd:bsd", v);
 *
 *   std::set<StringPiece> s;
 *   coral::splitTo<StringPiece>(":", "asd:bsd:asd:csd",
 ...
 * by delimiter. Any numeric type that coral::to<> can convert to from a
 ...
 * Examples:
 *
 *  coral::StringPiece name, key, value;
 *  if (coral::split('\t', line, name, key, value))
 *    ...
 *
 *  coral::StringPiece name;
 *  double value;
 *  int id;
 *  if (coral::split('\t', line, name, value, id))
 *    ...
 *
 ...
 *
 *  coral::StringPiece x, y.
 *  if (coral::split<false>(':', "a:b:c", x, y))
 *    assert(x == "a" && y == "b:c");
 *
 * Note that this will likely not work if the last field's target is of numeric
 * type, in which case coral::to<> will throw an exception.
 */
template <class T>
...
namespace boost {
template <class T>
struct has_nothrow_constructor<coral::basic_fbstring<T> > : true_type {
  enum { value = true };
};
} // namespace boost

#include <coral/String-inl.h>
