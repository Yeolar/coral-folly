...
char32_t decodeUtf8(
  ...
  if (p >= e) {
    ...
    throw std::runtime_error("coral::decodeUtf8 empty/invalid string");
  }
  ...
  if ((fst & 0xC0) != 0xC0) {
    if (skipOnError) return skip();
    throw std::runtime_error(to<std::string>("coral::decodeUtf8 i=0 d=", d));
  }
  ...
  for (unsigned int i = 1; i != 3 && p + i < e; ++i) {
    ...
    if ((tmp & 0xC0) != 0x80) {
      ...
      throw std::runtime_error(
        to<std::string>("coral::decodeUtf8 i=", i, " tmp=", (uint32_t)tmp));
    }
    ...
    if (!(fst & 0x80)) {
      ...
      if ((d & ~bitMask[i - 1]) == 0) {
        if (skipOnError) return skip();
        throw std::runtime_error(
          to<std::string>("coral::decodeUtf8 i=", i, " d=", d));
      }
  ...
  if (skipOnError) return skip();
  throw std::runtime_error("coral::decodeUtf8 encoding length maxed out");
}
  ...
  void operator()(dynamic const& v) const {
    ...
    case dynamic::DOUBLE:
      if (!opts_.allow_nan_inf &&
        ...
        throw std::runtime_error("coral::toJson: JSON object value was a "
          "NaN or INF");
      }
    ...
    case dynamic::INT64: {
      ...
      if (opts_.javascript_safe) {
        // Use coral::to to check that this integer can be represented
...
private:
  void printKV(const std::pair<const dynamic, dynamic>& p) const {
    if (!opts_.allow_non_string_keys && !p.first.isString()) {
      throw std::runtime_error("coral::toJson: JSON object key was not a "
        "string");
    }
    (*this)(p.first);
    mapColon();
...
struct Input {
  ...
  void skipWhitespace() {
    range_ = coral::skipWhitespace(range_);
    storeCurrent();
  }
