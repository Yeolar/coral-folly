...
static void testGrouping(const char* a_str, const char* expected) {
  ...
  coral::detail::insertThousandsGroupingUnsafe(str, &end_ptr);
 ...
 public:
  ...
  void format(FormatArg& arg, FormatCallback& cb) const {
    format_value::formatFormatter(
        coral::format("<key={}, value={}>", kv_.key, kv_.value),
        arg, cb);
  }
  ...
  do { \
    ...
    try { \
      ...
      ADD_FAILURE() << "Caught different exception type; expected " #type \
        ", caught " << coral::demangle(typeid(e)); \
    } catch (...) { \
      caught = true; \
...
#define EXPECT_FORMAT_ERROR(code, str) \
  EXPECT_THROW_STR(code, coral::BadFormatArg, (str))
