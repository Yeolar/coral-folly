...
// a useful runtime representation of the coral::dynamic.  We will
...
void dynamic::print_as_pseudo_json(std::ostream& out) const {
  out << "<coral::dynamic object of type " << type_ << ">";
}
...
TEST(Dynamic, ObjectBasics) {
  ...
  std::vector<std::pair<coral::fbstring, dynamic>> found;
  ...
  dynamic objInsert = coral::dynamic::object();
  dynamic objA = coral::dynamic::object("1", "2");
  dynamic objB = coral::dynamic::object("1", "2");
...
coral::fbstring make_long_string() {
  return coral::fbstring(100, 'a');
}
