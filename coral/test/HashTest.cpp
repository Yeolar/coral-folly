...
#include <coral/MapUtil.h>
#include <gtest/gtest.h>
#include <stdint.h>
#include <unordered_map>
#include <utility>

using namespace coral::hash;
...
TEST(Hash, hasher) {
  ...
  std::unordered_map<int32_t,int32_t,coral::hasher<int32_t>> m;
...
TEST(Hash, enum_type) {
  ...
  std::unordered_map<Enum32, std::string, coral::Hash> m32;
  m32[Enum32::Foo] = "foo";
  ...
  std::unordered_map<Enum64, std::string, coral::Hash> m64;
  m64[Enum64::Foo] = "foo";
  EXPECT_EQ("foo", m64[Enum64::Foo]);
}

TEST(Hash, pair_coral_hash) {
  typedef std::pair<int64_t, int32_t> pair2;
  pair2 p(42, 1);

  std::unordered_map<pair2, std::string, coral::Hash> m;
  m[p] = "bar";
  EXPECT_EQ("bar", m[p]);
}

TEST(Hash, tuple_coral_hash) {
  typedef std::tuple<int64_t, int32_t, int32_t> tuple3;
  tuple3 t(42, 1, 1);

  std::unordered_map<tuple3, std::string, coral::Hash> m;
  m[t] = "bar";
  EXPECT_EQ("bar", m[t]);
}
