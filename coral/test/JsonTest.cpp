...
TEST(Json, ParseTrailingComma) {
  coral::json::serialization_opts on, off;
...
TEST(Json, JavascriptSafe) {
  ...
  EXPECT_EQ(coral::toJson(badDouble), coral::to<coral::fbstring>(badDouble));
  coral::json::serialization_opts opts;
  opts.javascript_safe = true;
  EXPECT_ANY_THROW(coral::json::serialize(badDouble, opts));

  auto okDouble = 1ll << 63ll;
  dynamic okDyn = okDouble;
  EXPECT_EQ(coral::toJson(okDouble), coral::to<coral::fbstring>(okDouble));
}
...
TEST(Json, Produce) {
  ...
  EXPECT_EQ("Infinity",
            coral::json::serialize(parseJson("Infinity"), opts).toStdString());
  EXPECT_EQ("NaN",
            coral::json::serialize(parseJson("NaN"), opts).toStdString());
}

TEST(Json, JsonEscape) {
  coral::json::serialization_opts opts;
  EXPECT_EQ(
    coral::json::serialize("\b\f\n\r\x01\t\\\"/\v\a", opts),
    R"("\b\f\n\r\u0001\t\\\"/\u000b\u0007")");
}
...
TEST(Json, JsonNonAsciiEncoding) {
  ...
  // simple tests
  EXPECT_EQ(coral::json::serialize("\x1f", opts), R"("\u001f")");
  EXPECT_EQ(coral::json::serialize("\xc2\xa2", opts), R"("\u00a2")");
  EXPECT_EQ(coral::json::serialize("\xe2\x82\xac", opts), R"("\u20ac")");

  // multiple unicode encodings
  EXPECT_EQ(
    coral::json::serialize("\x1f\xe2\x82\xac", opts),
    R"("\u001f\u20ac")");
  EXPECT_EQ(
    coral::json::serialize("\x1f\xc2\xa2\xe2\x82\xac", opts),
    R"("\u001f\u00a2\u20ac")");
  EXPECT_EQ(
    coral::json::serialize("\xc2\x80\xef\xbf\xbf", opts),
    R"("\u0080\uffff")");
  EXPECT_EQ(
    coral::json::serialize("\xe0\xa0\x80\xdf\xbf", opts),
    R"("\u0800\u07ff")");

  // first possible sequence of a certain length
  EXPECT_EQ(coral::json::serialize("\xc2\x80", opts), R"("\u0080")");
  EXPECT_EQ(coral::json::serialize("\xe0\xa0\x80", opts), R"("\u0800")");

  // last possible sequence of a certain length
  EXPECT_EQ(coral::json::serialize("\xdf\xbf", opts), R"("\u07ff")");
  EXPECT_EQ(coral::json::serialize("\xef\xbf\xbf", opts), R"("\uffff")");

  // other boundary conditions
  EXPECT_EQ(coral::json::serialize("\xed\x9f\xbf", opts), R"("\ud7ff")");
  EXPECT_EQ(coral::json::serialize("\xee\x80\x80", opts), R"("\ue000")");
  EXPECT_EQ(coral::json::serialize("\xef\xbf\xbd", opts), R"("\ufffd")");

  // incomplete sequences
  EXPECT_ANY_THROW(coral::json::serialize("a\xed\x9f", opts));
  EXPECT_ANY_THROW(coral::json::serialize("b\xee\x80", opts));
  EXPECT_ANY_THROW(coral::json::serialize("c\xef\xbf", opts));

  // impossible bytes
  EXPECT_ANY_THROW(coral::json::serialize("\xfe", opts));
  EXPECT_ANY_THROW(coral::json::serialize("\xff", opts));

  // Sample overlong sequences
  EXPECT_ANY_THROW(coral::json::serialize("\xc0\xaf", opts));
  EXPECT_ANY_THROW(coral::json::serialize("\xe0\x80\xaf", opts));

  // Maximum overlong sequences
  EXPECT_ANY_THROW(coral::json::serialize("\xc1\xbf", opts));
  EXPECT_ANY_THROW(coral::json::serialize("\x30\x9f\xbf", opts));

  // illegal code positions
  EXPECT_ANY_THROW(coral::json::serialize("\xed\xa0\x80", opts));
  EXPECT_ANY_THROW(coral::json::serialize("\xed\xbf\xbf", opts));

  // Overlong representation of NUL character
  EXPECT_ANY_THROW(coral::json::serialize("\xc0\x80", opts));
  EXPECT_ANY_THROW(coral::json::serialize("\xe0\x80\x80", opts));

  // Longer than 3 byte encodings
  EXPECT_ANY_THROW(coral::json::serialize("\xf4\x8f\xbf\xbf", opts));
  EXPECT_ANY_THROW(coral::json::serialize("\xed\xaf\xbf\xed\xbf\xbf", opts));
}

TEST(Json, UTF8Retention) {

  // test retention with valid utf8 strings
  coral::fbstring input = "\u2665";
  coral::fbstring jsonInput = coral::toJson(input);
  coral::fbstring output = coral::parseJson(jsonInput).asString();
  coral::fbstring jsonOutput = coral::toJson(output);
  ...
  EXPECT_EQ(
    coral::toJson("a\xe0\xa0\x80z\xc0\x80"),
...
TEST(Json, UTF8EncodeNonAsciiRetention) {
  ...
  coral::fbstring input = "\u2665";
  coral::fbstring jsonInput = coral::json::serialize(input, opts);
  coral::fbstring output = coral::parseJson(jsonInput).asString();
  coral::fbstring jsonOutput = coral::json::serialize(output, opts);
  ...
  EXPECT_ANY_THROW(coral::json::serialize("a\xe0\xa0\x80z\xc0\x80", opts));
  EXPECT_ANY_THROW(coral::json::serialize("a\xe0\xa0\x80z\xe0\x80\x80", opts));
}
...
TEST(Json, UTF8Validation) {
  ...
  EXPECT_EQ(coral::json::serialize("a\xc2\x80z", opts), "\"a\xc2\x80z\"");
  EXPECT_EQ(
    coral::json::serialize("a\xe0\xa0\x80z", opts),
    "\"a\xe0\xa0\x80z\"");
  EXPECT_EQ(
    coral::json::serialize("a\xe0\xa0\x80m\xc2\x80z", opts),
  ...
  EXPECT_EQ(coral::json::serialize("a\xe0\xa0\x80z\xc0\x80", opts),
            "\"a\xe0\xa0\x80z\ufffd\ufffd\"");
  EXPECT_EQ(coral::json::serialize("a\xe0\xa0\x80z\xc0\x80\x80", opts),
            "\"a\xe0\xa0\x80z\ufffd\ufffd\ufffd\"");
  EXPECT_EQ(coral::json::serialize("z\xc0\x80z\xe0\xa0\x80", opts),
            "\"z\ufffd\ufffdz\xe0\xa0\x80\"");
...
TEST(Json, SortKeys) {
  coral::json::serialization_opts opts_on, opts_off;
  ...
  EXPECT_EQ(value, parseJson(coral::json::serialize(value, opts_on)));
  EXPECT_EQ(value, parseJson(coral::json::serialize(value, opts_off)));

  EXPECT_EQ(sorted_keys, coral::json::serialize(value, opts_on));
}
