...
TEST(Conv, digits10Minimal) {
  ...
  coral::doNotOptimizeAway(digits10(random() * random()));
}
...
TEST(Conv, Type2Type) {
  ...
  coral::fbstring fbStrV = "FBString";
  EXPECT_EQ(to<coral::fbstring>(fbStrV), "FBString");

  coral::StringPiece spV("StringPiece");
  EXPECT_EQ(to<coral::StringPiece>(spV), "StringPiece");
  ...
  EXPECT_EQ(to<coral::fbstring>(coral::fbstring("hello")), "hello");
  EXPECT_EQ(to<coral::StringPiece>(coral::StringPiece("Forty Two")),
            "Forty Two");
}
...
TEST(Conv, allocate_size) {
  ...
  auto res1 = coral::to<std::string>(str1, ".", str2);
...
void coralAtoiMeasure(unsigned int n, unsigned int digits) {
  auto p = pc1.subpiece(pc1.size() - digits, digits);
  FOR_EACH_RANGE (i, 0, n) {
    doNotOptimizeAway(coral::to<int64_t>(p.begin(), p.end()));
  }
}
...
void u64ToAsciiCoralBM(unsigned int n, uint64_t value) {
  // This is too fast, need to do 10 times per iteration
  char buf[20];
  FOR_EACH_RANGE (i, 0, n) {
...
void u2aAppendCoralBM(unsigned int n, uint64_t value) {
  string s;
  FOR_EACH_RANGE (i, 0, n) {
    // auto buf = &s.back() + 1;
    char buffer[20];
...
#define DEFINE_BENCHMARK_GROUP(n)                       \
  ...
  BENCHMARK_RELATIVE_PARAM(u64ToAsciiCoralBM, n);       \
  BENCHMARK_DRAW_LINE();
...
#define DEFINE_BENCHMARK_GROUP(n)                       \
  ...
  BENCHMARK_RELATIVE_PARAM(coralAtoiMeasure, n);        \
  BENCHMARK_DRAW_LINE();
