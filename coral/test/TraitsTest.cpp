...
namespace coral {
  ...
  template <> CORAL_ASSUME_RELOCATABLE(T2);
}
...
TEST(Traits, is_negative) {
  EXPECT_TRUE(coral::is_negative(-1));
  EXPECT_FALSE(coral::is_negative(0));
  EXPECT_FALSE(coral::is_negative(1));
  EXPECT_FALSE(coral::is_negative(0u));
  EXPECT_FALSE(coral::is_negative(1u));

  EXPECT_TRUE(coral::is_non_positive(-1));
  EXPECT_TRUE(coral::is_non_positive(0));
  EXPECT_FALSE(coral::is_non_positive(1));
  EXPECT_TRUE(coral::is_non_positive(0u));
  EXPECT_FALSE(coral::is_non_positive(1u));
}
...
TEST(Traits, relational) {
  ...
  EXPECT_FALSE((coral::less_than<uint8_t, 0u,   uint8_t>(0u)));
  EXPECT_FALSE((coral::less_than<uint8_t, 0u,   uint8_t>(254u)));
  EXPECT_FALSE((coral::less_than<uint8_t, 255u, uint8_t>(255u)));
  EXPECT_TRUE( (coral::less_than<uint8_t, 255u, uint8_t>(254u)));

  EXPECT_FALSE((coral::greater_than<uint8_t, 0u,   uint8_t>(0u)));
  EXPECT_TRUE( (coral::greater_than<uint8_t, 0u,   uint8_t>(254u)));
  EXPECT_FALSE((coral::greater_than<uint8_t, 255u, uint8_t>(255u)));
  EXPECT_FALSE((coral::greater_than<uint8_t, 255u, uint8_t>(254u)));
}

struct membership_no {};
struct membership_yes { using x = void; };
CORAL_CREATE_HAS_MEMBER_TYPE_TRAITS(has_member_type_x, x);
