...
#include <coral/DynamicConverter.h>
...
using namespace coral::dynamicconverter_detail;
...
TEST(DynamicConverter, simple_builtins) {
  dynamic d1 = "Haskell";
  auto i1 = convertTo<coral::fbstring>(d1);
  ...
  auto i4 = convertTo<std::pair<std::string, coral::fbstring>>(d4);
...
TEST(DynamicConverter, simple_fbvector) {
  dynamic d1 = { 1, 2, 3 };
  auto i1 = convertTo<coral::fbvector<int>>(d1);
...
TEST(DynamicConverter, nested_containers) {
  dynamic d1 = { { 1 }, { }, { 2, 3 } };
  auto i1 = convertTo<coral::fbvector<std::vector<uint8_t>>>(d1);
  decltype(i1) i1b = { { 1 }, { }, { 2, 3 } };
  EXPECT_EQ(i1, i1b);
  ...
  auto i2 = convertTo<std::map<double, std::vector<coral::fbstring>>>(d2);
  decltype(i2) i2b =
    { { 3.14, { "3", ".", "1", "4" } },
      { 2.72, { "2", ".", "7", "2" } } };
  EXPECT_EQ(i2, i2b);
