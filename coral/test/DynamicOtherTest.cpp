...
using coral::dynamic;
using coral::TypeError;

TEST(Dynamic, ArrayGenerator) {
  // Make sure arrays can be used with coral::gen.
  using namespace coral::gen;
  dynamic arr { 1, 2, 3, 4 };
...
TEST(Dynamic, StringPtrs) {
  dynamic str = "12.0";
  dynamic num = 12.0;
  dynamic nullStr = coral::parseJson("\"foo\\u0000bar\"");
  ...
  EXPECT_EQ(nullStr.stringPiece(), coral::StringPiece("foo\0bar", 7));
...
TEST(Dynamic, Getters) {
  dynamic dStr = coral::parseJson("\"foo\\u0000bar\"");
  dynamic dInt = 1;
  dynamic dDouble = 0.5;
  dynamic dBool = true;
