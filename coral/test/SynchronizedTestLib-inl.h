...
#ifndef CORAL_TEST_SYNCHRONIZEDTESTLIB_INL_H
#define CORAL_TEST_SYNCHRONIZEDTESTLIB_INL_H
...
inline std::mt19937& getRNG() {
  static const auto seed = coral::randomNumberSeed();
...
void testBasic() {
  coral::Synchronized<std::vector<int>, Mutex> obj;
  ...
  EXPECT_EQ(CR_ARG_2_OR_1(1, 2), 2);
  EXPECT_EQ(CR_ARG_2_OR_1(1), 1);
}

template <class Mutex> void testConcurrency() {
  coral::Synchronized<std::vector<int>, Mutex> v;

  struct Local {
    static bool threadMain(int i,
                           coral::Synchronized<std::vector<int>, Mutex>& pv) {
...
template <class Mutex> void testDualLocking() {
  coral::Synchronized<std::vector<int>, Mutex> v;
  coral::Synchronized<std::map<int, int>, Mutex> m;

  struct Local {
    static bool threadMain(
      int i,
      coral::Synchronized<std::vector<int>, Mutex>& pv,
      coral::Synchronized<std::map<int, int>, Mutex>& pm) {
...
template <class Mutex> void testDualLockingWithConst() {
  ...
  struct Local {
    static bool threadMain(
      int i,
      coral::Synchronized<std::vector<int>, Mutex>& pv,
      const coral::Synchronized<std::map<int, int>, Mutex>& pm) {
...
template <class Mutex> void testConstCopy() {
  ...
  const coral::Synchronized<std::vector<int>, Mutex> v(input);
...
#endif  /* CORAL_TEST_SYNCHRONIZEDTESTLIB_INL_H */
