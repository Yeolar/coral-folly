...
TEST(SpinLock, MslCorrectness) {
  correctnessTest<coral::SpinLockMslImpl>();
}
TEST(SpinLock, MslTryLock) {
  trylockTest<coral::SpinLockMslImpl>();
}
#endif

#if __APPLE__
TEST(SpinLock, AppleCorrectness) {
  correctnessTest<coral::SpinLockAppleImpl>();
}
TEST(SpinLock, AppleTryLock) {
  trylockTest<coral::SpinLockAppleImpl>();
}
#endif

#if CORAL_HAVE_PTHREAD_SPINLOCK_T
TEST(SpinLock, PthreadCorrectness) {
  correctnessTest<coral::SpinLockPthreadImpl>();
}
TEST(SpinLock, PthreadTryLock) {
  trylockTest<coral::SpinLockPthreadImpl>();
}
#endif

TEST(SpinLock, MutexCorrectness) {
  correctnessTest<coral::SpinLockPthreadMutexImpl>();
}
TEST(SpinLock, MutexTryLock) {
  trylockTest<coral::SpinLockPthreadMutexImpl>();
}
