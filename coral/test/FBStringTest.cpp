...
static const int seed = coral::randomNumberSeed();
...
void Num2String(String& str, Integral n) {
  ...
  std::string tmp = coral::to<std::string>(n);
...
TEST(FBString, testAllClauses) {
  ...
  std::wstring wr;
  coral::fbstring c;
  coral::basic_fbstring<wchar_t> wc;
  int count = 0;

  auto l = [&](const char * const clause,
               void(*f_string)(std::string&),
               void(*f_fbstring)(coral::fbstring&),
               void(*f_wfbstring)(coral::basic_fbstring<wchar_t>&)) {
    do {
      ...
      wc = coral::basic_fbstring<wchar_t>(wr.c_str());
...
#define TEST_CLAUSE(x) \
  l(#x, \
    ...
    clause11_##x<coral::fbstring>, \
    clause11_##x<coral::basic_fbstring<wchar_t>>);
...
TEST(FBString, testFixedBugs) {
  ...
  { // D661622
    coral::basic_fbstring<wchar_t> s;
    EXPECT_EQ(0, s.size());
  }
  { // D785057
    fbstring str(1337, 'f');
