...
#include <coral/experimental/TestUtil.h>
...
#define EXPECT_CONTAINS(haystack, needle) \
  EXPECT_NE(::std::string::npos, ::coral::StringPiece(haystack).find(needle)) \
    << "Haystack: '" << haystack << "'\nNeedle: '" << needle << "'";
...
TEST(File, Locks) {
  ...
  if (fs::exists(me.parent_path() / helper_basename)) {
    ...
    throw std::runtime_error(
      coral::to<std::string>("cannot find helper ", helper_basename));
  }
