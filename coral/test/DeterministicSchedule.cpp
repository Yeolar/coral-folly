...
#include <coral/test/DeterministicSchedule.h>
#include <algorithm>
#include <list>
#include <mutex>
#include <random>
...
CORAL_TLS sem_t* DeterministicSchedule::tls_sem;
CORAL_TLS DeterministicSchedule* DeterministicSchedule::tls_sched;
CORAL_TLS unsigned DeterministicSchedule::tls_threadId;
...
void DeterministicSchedule::beforeThreadExit() {
  ...
  if (sems_.size() > 0) {
    CORAL_TEST_DSCHED_VLOG("exiting");
    afterSharedAccess();
  }
  sem_destroy(tls_sem);
  delete tls_sem;
...
void DeterministicSchedule::join(std::thread& child) {
  ...
  if (sched) {
    ...
    while (!done) {
      ...
      if (done) {
        CORAL_TEST_DSCHED_VLOG("joined " << std::hex << child.get_id());
      }
      afterSharedAccess();
    }
  }
...
void DeterministicSchedule::post(sem_t* sem) {
  beforeSharedAccess();
  sem_post(sem);
  CORAL_TEST_DSCHED_VLOG("sem_post(" << sem << ")");
  afterSharedAccess();
}
...
bool DeterministicSchedule::tryWait(sem_t* sem) {
  ...
  int e = rv == 0 ? 0 : errno;
  CORAL_TEST_DSCHED_VLOG("sem_trywait(" << sem << ") = " << rv
                                        << " errno=" << e);
  afterSharedAccess();
  if (rv == 0) {
    return true;
...
FutexResult Futex<DeterministicAtomic>::futexWaitImpl(
  ...
  CORAL_TEST_DSCHED_VLOG(this << ".futexWait(" << std::hex << expected
                              << ", .., " << std::hex << waitMask
                              << ") beginning..");
  futexLock.lock();
  if (data == expected) {
...
int Futex<DeterministicAtomic>::futexWake(int count, uint32_t wakeMask) {
  ...
  futexLock.unlock();
  CORAL_TEST_DSCHED_VLOG(this << ".futexWake(" << count << ", " << std::hex
                              << wakeMask << ") -> " << rv);
  DeterministicSchedule::afterSharedAccess();
  return rv;
}
