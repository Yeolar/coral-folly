...
TEST(ApplyTuple, Test) {
  ...
  auto func2 = func;
  coral::applyTuple(func2, argsTuple);
  coral::applyTuple(func, argsTuple);
  coral::applyTuple(func, std::make_tuple(1, 2, 3.0));
  coral::applyTuple(makeFunc(), std::make_tuple(1, 2, 3.0));
  coral::applyTuple(makeFunc(), argsTuple);

  std::unique_ptr<Wat> wat(new Wat);
  coral::applyTuple(&Wat::func, std::make_tuple(wat.get(), 1, 2, 3.0));
  auto argsTuple2 = std::make_tuple(wat.get(), 1, 2, 3.0);
  coral::applyTuple(&Wat::func, argsTuple2);

  EXPECT_EQ(10.0,
            coral::applyTuple(&Wat::retVal,
  ...
  int x = coral::applyTuple(std::plus<int>(), std::make_tuple(12, 12));
  EXPECT_EQ(24, x);

  Mover m;
  coral::applyTuple(move_only_func,
                    std::forward_as_tuple(std::forward<Mover>(Mover())));
  const auto tuple3 = std::make_tuple(1, 2, 3.0);
  coral::applyTuple(func, tuple3);
}
