...
#include <coral/detail/CacheLocality.h>
...
#define CORAL_TEST_DSCHED_VLOG(msg...)                                  \
  do {                                                                  \
    if (false) {                                                        \
...
class DeterministicSchedule : boost::noncopyable {
  ...
  static inline std::thread thread(Func&& func, Args&&... args) {
    ...
    auto child = std::thread([=](Args... a) {
      if (sched) {
        ...
        beforeSharedAccess();
        CORAL_TEST_DSCHED_VLOG("running");
        afterSharedAccess();
      }
      SCOPE_EXIT {
        if (sched) {
    ...
    if (sched) {
      ...
      CORAL_TEST_DSCHED_VLOG("forked " << std::hex << child.get_id());
      afterSharedAccess();
    }
    return child;
  }
 ...
 private:
  static CORAL_TLS sem_t* tls_sem;
  static CORAL_TLS DeterministicSchedule* tls_sched;
  static CORAL_TLS unsigned tls_threadId;
...
struct DeterministicAtomic {
  ...
  bool compare_exchange_strong(
    ...
    CORAL_TEST_DSCHED_VLOG(this << ".compare_exchange_strong(" << std::hex
                                << orig << ", " << std::hex << v1 << ") -> "
                                << rv << "," << std::hex << v0);
  ...
  bool compare_exchange_weak(
    ...
    CORAL_TEST_DSCHED_VLOG(this << ".compare_exchange_weak(" << std::hex << orig
                                << ", " << std::hex << v1 << ") -> " << rv
                                << "," << std::hex << v0);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  ...
  T exchange(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    ...
    CORAL_TEST_DSCHED_VLOG(this << ".exchange(" << std::hex << v << ") -> "
                                << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  /* implicit */ operator T() const noexcept {
    DeterministicSchedule::beforeSharedAccess();
    T rv = data;
    CORAL_TEST_DSCHED_VLOG(this << "() -> " << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T load(std::memory_order mo = std::memory_order_seq_cst) const noexcept {
    DeterministicSchedule::beforeSharedAccess();
    T rv = data.load(mo);
    CORAL_TEST_DSCHED_VLOG(this << ".load() -> " << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T operator=(T v) noexcept {
    DeterministicSchedule::beforeSharedAccess();
    T rv = (data = v);
    CORAL_TEST_DSCHED_VLOG(this << " = " << std::hex << v);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  void store(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    DeterministicSchedule::beforeSharedAccess();
    data.store(v, mo);
    CORAL_TEST_DSCHED_VLOG(this << ".store(" << std::hex << v << ")");
    DeterministicSchedule::afterSharedAccess();
  }
  ...
  T operator++() noexcept {
    DeterministicSchedule::beforeSharedAccess();
    T rv = ++data;
    CORAL_TEST_DSCHED_VLOG(this << " pre++ -> " << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T operator++(int postDummy) noexcept {
    DeterministicSchedule::beforeSharedAccess();
    T rv = data++;
    CORAL_TEST_DSCHED_VLOG(this << " post++ -> " << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T operator+=(T v) noexcept {
    DeterministicSchedule::beforeSharedAccess();
    T rv = (data += v);
    CORAL_TEST_DSCHED_VLOG(this << " += " << std::hex << v << " -> " << std::hex
                                << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T fetch_add(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    ...
    T rv = data;
    data += v;
    CORAL_TEST_DSCHED_VLOG(this << ".fetch_add(" << std::hex << v << ") -> "
                                << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T fetch_sub(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    ...
    T rv = data;
    data -= v;
    CORAL_TEST_DSCHED_VLOG(this << ".fetch_sub(" << std::hex << v << ") -> "
                                << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T fetch_and(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    ...
    T rv = data;
    data &= v;
    CORAL_TEST_DSCHED_VLOG(this << ".fetch_and(" << std::hex << v << ") -> "
                                << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T fetch_or(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    ...
    T rv = data;
    data |= v;
    CORAL_TEST_DSCHED_VLOG(this << ".fetch_or(" << std::hex << v << ") -> "
                                << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
  ...
  T fetch_xor(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    ...
    T rv = data;
    data ^= v;
    CORAL_TEST_DSCHED_VLOG(this << ".fetch_xor(" << std::hex << v << ") -> "
                                << std::hex << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }
