 ...
 */
class FiberManager : public ::coral::Executor {
 public:
  struct Options {
 ...
 private:
  ...
  struct RemoteTask {
    ...
    RemoteTask(F&& f, const Fiber::LocalData& localData_) :
        func(std::forward<F>(f)),
        localData(coral::make_unique<Fiber::LocalData>(localData_)),
        rcontext(RequestContext::saveContext()) {}
  ...
  typedef coral::IntrusiveList<Fiber, &Fiber::listHook_> FiberTailQueue;
   ...
   * it's nullptr
   */
  static CORAL_TLS FiberManager* currentFiberManager_;
  ...
  coral::AtomicLinkedList<Fiber, &Fiber::nextRemoteReady_> remoteReadyQueue_;

  coral::AtomicLinkedList<RemoteTask, &RemoteTask::nextRemoteTask>
      remoteTaskQueue_;
