...
TEST(FiberManager, batonTimedWaitTimeout) {
  ...
  FiberManager manager(coral::make_unique<SimpleLoopController>());
  auto& loopController =
...
TEST(FiberManager, batonTimedWaitTimeoutEvb) {
  ...
  FiberManager manager(coral::make_unique<EventBaseLoopController>());
          ...
          for (size_t i = 0; i < 3; ++i) {
            funcs.push_back(
              [i, &pendingFibers]() {
                ...
                return coral::make_unique<int>(i*2 + 1);
              }
            );
          }
...
TEST(FiberManager, fibersPoolWithinLimit) {
  ...
  FiberManager manager(coral::make_unique<SimpleLoopController>(), opts);
  auto& loopController =
    dynamic_cast<SimpleLoopController&>(manager.loopController());
...
TEST(FiberManager, remoteFiberBasic) {
  ...
  int result[2];
  result[0] = result[1] = 0;
  coral::Optional<Promise<int>> savedPromise[2];
  manager.addTask(
    [&] () {
...
TEST(FiberManager, remoteHasTasks) {
  size_t counter = 0;
  FiberManager fm(coral::make_unique<SimpleLoopController>());
  std::thread remote([&]() {
    fm.addTaskRemote([&]() {
      ++counter;
    });
...
TEST(FiberManager, remoteHasReadyTasks) {
  int result = 0;
  coral::Optional<Promise<int>> savedPromise;
  FiberManager fm(coral::make_unique<SimpleLoopController>());
  std::thread remote([&]() {
    fm.addTaskRemote([&]() {
...
void testFiberLocal() {
  FiberManager fm(LocalType<Data>(),
                  coral::make_unique<SimpleLoopController>());
...
TEST(FiberManager, RequestContext) {
  ...
  coral::fibers::Baton baton1;
  coral::fibers::Baton baton2;
  coral::fibers::Baton baton3;

  coral::RequestContext::create();
  auto rcontext1 = coral::RequestContext::get();
  fm.addTask([&]() {
      EXPECT_EQ(rcontext1, coral::RequestContext::get());
      baton1.wait([&]() {
          EXPECT_EQ(rcontext1, coral::RequestContext::get());
        });
      EXPECT_EQ(rcontext1, coral::RequestContext::get());
  ...
  coral::RequestContext::create();
  auto rcontext2 = coral::RequestContext::get();
  fm.addTaskRemote([&]() {
      EXPECT_EQ(rcontext2, coral::RequestContext::get());
      baton2.wait();
      EXPECT_EQ(rcontext2, coral::RequestContext::get());
      checkRun2 = true;
    });
  ...
  coral::RequestContext::create();
  auto rcontext3 = coral::RequestContext::get();
  fm.addTaskFinally([&]() {
      EXPECT_EQ(rcontext3, coral::RequestContext::get());
      baton3.wait();
      EXPECT_EQ(rcontext3, coral::RequestContext::get());
  ...
  coral::RequestContext::create();
  auto rcontext = coral::RequestContext::get();

  fm.loopUntilNoReady();
  EXPECT_EQ(rcontext, coral::RequestContext::get());
...
void runBenchmark(size_t numAwaits, size_t toSend) {
  ...
  FiberManager fiberManager(coral::make_unique<SimpleLoopController>());
  auto& loopController =
