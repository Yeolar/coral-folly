...
class StackCache {
 public:
  ...
  unsigned char* borrow(size_t size) {
    std::lock_guard<coral::SpinLock> lg(lock_);
...
class CacheManager {
 public:
  ...
  std::unique_ptr<StackCacheEntry> getStackCache(size_t stackSize) {
    ...
    if (inUse_ < kMaxInUse) {
      ++inUse_;
      return coral::make_unique<StackCacheEntry>(stackSize);
    }
 ...
 private:
  ...
  void giveBack(std::unique_ptr<StackCache> stackCache_) {
    ...
    /* Note: we can add a free list for each size bucket
       ...
       In this case this needs to be a coral::Singleton
...
class StackCacheEntry {
 public:
  explicit StackCacheEntry(size_t stackSize)
      : stackCache_(coral::make_unique<StackCache>(stackSize)) {
  }
