...
#ifndef CORAL_LIFOSEM_H
#define CORAL_LIFOSEM_H
...
#include <coral/AtomicStruct.h>
#include <coral/Baton.h>
#include <coral/IndexedMemPool.h>
...
struct LifoSemRawNode {
  ...
  typedef coral::IndexedMemPool<LifoSemRawNode<Atom>,32,200,Atom> Pool;
...
#define LIFOSEM_DECLARE_POOL(Atom, capacity)                       \
    template<>                                                     \
    coral::detail::LifoSemRawNode<Atom>::Pool                      \
        coral::detail::LifoSemRawNode<Atom>::pool((capacity));
...
struct LifoSemBase : boost::noncopyable {
 ...
 private:
  ...
  CORAL_ALIGN_TO_AVOID_FALSE_SHARING
  coral::AtomicStruct<LifoSemHead,Atom> head_;

  char padding_[coral::detail::CacheLocality::kFalseSharingRange -
      sizeof(LifoSemHead)];
