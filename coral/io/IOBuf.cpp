...
void takeOwnershipError(bool freeOnError, void* buf,
                        coral::IOBuf::FreeFunction freeFn,
                        void* userData) {
  if (!freeOnError) {
    return;
  }
...
void IOBuf::reserveSlow(uint64_t minHeadroom, uint64_t minTailroom) {
  ...
  //     headroom + data + tailroom, see smartRealloc in coral/Malloc.h)
...
coral::fbvector<struct iovec> IOBuf::getIov() const {
  coral::fbvector<struct iovec> iov;
  iov.reserve(countChainElements());
  appendToIov(&iov);
  return iov;
}

void IOBuf::appendToIov(coral::fbvector<struct iovec>* iov) const {
  IOBuf const* p = this;
  do {
    // some code can get confused by empty iovs, so skip them
    if (p->length() > 0) {
      iov->push_back({(void*)p->data(), coral::to<size_t>(p->length())});
    }
    p = p->next();
  } while (p != this);
}
...
size_t IOBufHash::operator()(const IOBuf& buf) const {
  coral::hash::SpookyHashV2 hasher;
  hasher.Init(0, 0);
