...
int AsyncServerSocket::createSocket(int family) {
  ...
  if (fd == -1) {
    coral::throwSystemError(errno, "error creating async server socket");
  }
...
void AsyncServerSocket::setupSocket(int fd) {
  ...
  if (closeOnExec_ &&
      (-1 == coral::setCloseOnExec(fd, closeOnExec_))) {
...
void AsyncServerSocket::dispatchError(const char *msgstr, int errnoValue) {
  ...
  while (true) {
    ...
    if (info->eventBase == nullptr) {
      std::runtime_error ex(
        std::string(msgstr) +  coral::to<std::string>(errnoValue));
