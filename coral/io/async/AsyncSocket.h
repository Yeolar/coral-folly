...
class AsyncSocket : virtual public AsyncTransportWrapper {
 ...
 protected:
  ...
  class ImmediateReadCB : public coral::EventBase::LoopCallback {
   public:
  ...
  void writeChainImpl(WriteCallback* callback, iovec* vec,
                      size_t count, std::unique_ptr<coral::IOBuf>&& buf,
                      WriteFlags flags);
  ...
  mutable
    coral::SocketAddress addr_;    ///< The address we tried to connect to
