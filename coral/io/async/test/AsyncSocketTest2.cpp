...
TEST(AsyncSocketTest, ConnectRefused) {
  ...
  coral::SocketAddress addr("127.0.0.1", 65535);
  ConnCallback cb;
...
TEST(AsyncSocketTest, ConnectTimeout) {
  ...
  coral::SocketAddress addr("8.8.8.8", 65535);
  ConnCallback cb;
...
class AsyncSocketImmediateRead : public coral::AsyncSocket {
 public:
  bool immediateReadCalled = false;
  explicit AsyncSocketImmediateRead(coral::EventBase* evb) : AsyncSocket(evb) {}
 protected:
...
class TestAcceptCallback : public AsyncServerSocket::AcceptCallback {
 public:
  ...
  struct EventInfo {
    EventInfo(int fd, const coral::SocketAddress& addr)
      : type(TYPE_ACCEPT),
        fd(fd),
        address(addr),
        errorMsg() {}
    ...
    coral::SocketAddress address;  // valid for TYPE_ACCEPT
  ...
  void setConnectionAcceptedFn(
      const std::function<void(int, const coral::SocketAddress&)>& fn) {
  ...
  void connectionAccepted(
      int fd, const coral::SocketAddress& clientAddr) noexcept override {
 ...
 private:
  std::function<void(int, const coral::SocketAddress&)> connectionAcceptedFn_;
...
TEST(AsyncSocketTest, ServerAcceptOptions) {
  ...
  coral::SocketAddress serverAddress;
  ...
  acceptCallback.setConnectionAcceptedFn(
    [&](int fd, const coral::SocketAddress& addr) {
...
TEST(AsyncSocketTest, RemoveAcceptCallback) {
  ...
  // called.
  int cb2Count = 0;
  cb1.setConnectionAcceptedFn([&](int fd, const coral::SocketAddress& addr){
      std::shared_ptr<AsyncSocket> sock2(
        AsyncSocket::newSocket(&eventBase, serverAddress)); // cb2: -cb3 -cb5
      });
  cb3.setConnectionAcceptedFn([&](int fd, const coral::SocketAddress& addr){
    });
  cb4.setConnectionAcceptedFn([&](int fd, const coral::SocketAddress& addr){
      std::shared_ptr<AsyncSocket> sock3(
        AsyncSocket::newSocket(&eventBase, serverAddress)); // cb4
    });
  cb5.setConnectionAcceptedFn([&](int fd, const coral::SocketAddress& addr){
...
TEST(AsyncSocketTest, ServerExistingSocket) {
  ...
  // then letting AsyncServerSocket listen
  {
    ...
    coral::SocketAddress boundAddress;
    ...
    coral::SocketAddress serverSocketAddress;
