 ...
 * under the License.
 */
#include <coral/io/async/test/SocketPair.h>
...
SocketPair::SocketPair(Mode mode) {
  if (socketpair(PF_UNIX, SOCK_STREAM, 0, fds_) != 0) {
    throw std::runtime_error(
      coral::to<std::string>("test::SocketPair: failed create socket pair",
                             errno));
  }
  ...
  if (mode == NONBLOCKING) {
    if (fcntl(fds_[0], F_SETFL, O_NONBLOCK) != 0) {
      throw std::runtime_error(
        coral::to<std::string>("test::SocketPair: failed to set non-blocking "
                               "read mode", errno));
    }
