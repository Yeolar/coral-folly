...
public:
  ...
  void connectionAccepted(int fd, const coral::SocketAddress& clientAddr)
    noexcept override{
  ...
  virtual void connAccepted(
    const std::shared_ptr<coral::AsyncSSLSocket> &s) = 0;
...
public:
  ...
  void connAccepted(
    const std::shared_ptr<coral::AsyncSSLSocket> &s)
    noexcept override {
...
class TestSSLServer {
 protected:
  ...
  std::shared_ptr<coral::AsyncServerSocket> socket_;
...
class SNIServer :
 ...
 public:
  explicit SNIServer(
    ...
    const std::shared_ptr<coral::SSLContext>& sniCtx,
 ...
 private:
  ...
  coral::SSLContext::ServerNameCallbackResult
    ...
    if (sniCtx_ &&
      ...
      return coral::SSLContext::SERVER_NAME_FOUND;
    } else {
      serverNameMatch = false;
      return coral::SSLContext::SERVER_NAME_NOT_FOUND;
    }
  }

  AsyncSSLSocket::UniquePtr socket_;
  std::shared_ptr<coral::SSLContext> sniCtx_;
                  ...
                  public AsyncTransportWrapper::ReadCallback
{
 ...
 public:
  SSLClient(EventBase *eventBase,
    ...
    ctx_.reset(new coral::SSLContext());
 ...
 public:
  SSLHandshakeClientNoVerify(
   ...
   bool verifyResult) :
    SSLHandshakeBase(std::move(socket), preverifyResult, verifyResult) {
    socket_->sslConn(this, 0,
      coral::SSLContext::SSLVerifyPeerEnum::NO_VERIFY);
  }
};
 ...
 public:
  SSLHandshakeClientDoVerify(
   ...
   bool verifyResult) :
    SSLHandshakeBase(std::move(socket), preverifyResult, verifyResult) {
    socket_->sslConn(this, 0,
      coral::SSLContext::SSLVerifyPeerEnum::VERIFY);
  }
};
...
class SSLHandshakeServerDoVerify : public SSLHandshakeBase {
 public:
  SSLHandshakeServerDoVerify(
    ...
    : SSLHandshakeBase(std::move(socket), preverifyResult, verifyResult) {
    socket_->sslAccept(this, 0,
      coral::SSLContext::SSLVerifyPeerEnum::VERIFY_REQ_CLIENT_CERT);
  }
};
