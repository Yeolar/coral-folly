 ...
 */
#include <coral/io/async/test/AsyncSSLSocketTest.h>
...
#include <coral/io/async/test/BlockingSocket.h>
...
const char* testCert = "coral/io/async/test/certs/tests-cert.pem";
const char* testKey = "coral/io/async/test/certs/tests-key.pem";
const char* testCA = "coral/io/async/test/certs/ca-cert.pem";
...
ctx_(new coral::SSLContext),
    acb_(acb),
  socket_(coral::AsyncServerSocket::newSocket(&evb_)) {
...
void getctx(
  std::shared_ptr<coral::SSLContext> clientCtx,
  std::shared_ptr<coral::SSLContext> serverCtx) {
...
void sslsocketpair(
  ...
  auto clientCtx = std::make_shared<coral::SSLContext>();
  auto serverCtx = std::make_shared<coral::SSLContext>();
  int fds[2];
  getfds(fds);
...
TEST(AsyncSSLSocketTest, SSLParseClientHelloOnePacket) {
  ...
  buf->append(bufLen);
  coral::io::RWPrivateCursor cursor(buf.get());
...
TEST(AsyncSSLSocketTest, SSLParseClientHelloTwoPackets) {
  ...
  auto bufCopy = coral::IOBuf::copyBuffer(buf->data(), 2);
  AsyncSSLSocket::clientHelloParsingCallback(
      0, 0, SSL3_RT_HANDSHAKE, bufCopy->data(), bufCopy->length(),
      ssl, sock.get());
  bufCopy.reset();
  bufCopy = coral::IOBuf::copyBuffer(buf->data() + 2, buf->length() - 2);
  AsyncSSLSocket::clientHelloParsingCallback(
      0, 0, SSL3_RT_HANDSHAKE, bufCopy->data(), bufCopy->length(),
      ssl, sock.get());
  bufCopy.reset();
...
TEST(AsyncSSLSocketTest, SSLParseClientHelloMultiplePackets) {
  ...
  for (uint64_t i = 0; i < buf->length(); i += 3) {
    auto bufCopy = coral::IOBuf::copyBuffer(
