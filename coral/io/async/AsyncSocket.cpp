...
const coral::SocketAddress& AsyncSocket::anyAddress() {
  static const coral::SocketAddress anyAddress =
    coral::SocketAddress("0.0.0.0", 0);
  return anyAddress;
}
...
void AsyncSocket::connect(ConnectCallback* callback,
                           ...
                           const coral::SocketAddress& bindAddr) noexcept {
...
void AsyncSocket::connect(ConnectCallback* callback,
  ...
  try {
    connectCallback_ = callback;
    connect(callback, coral::SocketAddress(ip, port), timeout, options);
  } catch (const std::exception& ex) {
    AsyncSocketException tex(AsyncSocketException::INTERNAL_ERROR,
                            ex.what());
    return failConnect(__func__, tex);
...
void AsyncSocket::getLocalAddress(coral::SocketAddress* address) const {
  address->setFromLocalAddress(fd_);
}

void AsyncSocket::getPeerAddress(coral::SocketAddress* address) const {
    ...
    if (bytesRead > 0) {
      if (!isBufferMovable_) {
        ...
        VLOG(5) << "this=" << this << ", AsyncSocket::handleRead() got "
                ...
                << ", offset=" << offset;
        auto readBuf = coral::IOBuf::takeOwnership(buf, buflen);
...
std::string AsyncSocket::withAddr(const std::string& s) {
  ...
  coral::SocketAddress peer, local;
  try {
    getPeerAddress(&peer);
