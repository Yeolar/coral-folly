...
#include <coral/io/async/AsyncSocketBase.h>
...
class AsyncUDPSocket : public EventHandler {
 public:
  ...
  class ReadCallback {
   public:
     ...
     */
    virtual void onDataAvailable(const coral::SocketAddress& client,
                                 size_t len,
   ...
   */
  virtual const coral::SocketAddress& address() const {
   ...
   * returns.
   */
  virtual void bind(const coral::SocketAddress& address);
   ...
   * ::sendmsg.
   */
  virtual ssize_t write(const coral::SocketAddress& address,
                        const std::unique_ptr<coral::IOBuf>& buf);

  /**
   * Send data in iovec to destination. Returns the return code from sendmsg.
   */
  virtual ssize_t writev(const coral::SocketAddress& address,
 ...
 private:
  ...
  coral::SocketAddress localAddress_;
