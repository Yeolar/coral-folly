...
class SSLException: public coral::AsyncSocketException {
 public:
...
class AsyncSSLSocket : public virtual AsyncSocket {
 public:
   ...
   */
  AsyncSSLSocket(const std::shared_ptr<coral::SSLContext> &ctx,
  ...
  virtual void sslAccept(HandshakeCB* callback, uint32_t timeout = 0,
      const coral::SSLContext::SSLVerifyPeerEnum& verifyPeer =
            coral::SSLContext::SSLVerifyPeerEnum::USE_CTX);
  ...
  void connect(ConnectCallback* callback,
               ...
               const coral::SocketAddress& bindAddr = anyAddress())
               noexcept override;
   ...
   */
  void attachSSLContext(const std::shared_ptr<coral::SSLContext>& ctx);
  ...
  void switchServerSSLContext(
    const std::shared_ptr<coral::SSLContext>& handshakeCtx);
  ...
  std::string getSSLClientComprMethods() {
    ...
    return coral::join(":", clientHelloInfo_->clientHelloCompressionMethods_);
  }
  ...
  std::string getSSLClientExts() {
    ...
    return coral::join(":", clientHelloInfo_->clientHelloExtensions_);
  }
  ...
  struct ClientHelloInfo {
    coral::IOBufQueue clientHelloBuf_;
 ...
 protected:
  ...
  std::shared_ptr<coral::SSLContext> ctx_;
...
#if OPENSSL_VERSION_NUMBER >= 0x1000105fL && !defined(OPENSSL_NO_TLSEXT)
  std::shared_ptr<coral::SSLContext> handshakeCtx_;
  std::string tlsextHostname_;
#endif
  coral::SSLContext::SSLVerifyPeerEnum
    verifyPeer_{coral::SSLContext::SSLVerifyPeerEnum::USE_CTX};
