...
#include <coral/io/async/EventHandler.h>
...
#define CORAL_HAVE_EVENTFD
#include <coral/io/async/EventFDWrapper.h>
#endif
 ...
 public:
  ...
  explicit NotificationQueue(uint32_t maxSize = 0,
    ...
    if (fdType == FdType::EVENTFD) {
      ...
      if (eventfd_ == -1) {
        if (errno == ENOSYS || errno == EINVAL) {
          ...
          // some other error
          coral::throwSystemError("Failed to create eventfd for "
    ...
    if (fdType == FdType::PIPE) {
      if (pipe(pipeFds_)) {
        coral::throwSystemError("Failed to create pipe for NotificationQueue",
                                errno);
      }
      try {
        // put both ends of the pipe into non-blocking mode
        if (fcntl(pipeFds_[0], F_SETFL, O_RDONLY | O_NONBLOCK) != 0) {
          coral::throwSystemError("failed to put NotificationQueue pipe read "
                                  "endpoint into non-blocking mode", errno);
        }
        if (fcntl(pipeFds_[1], F_SETFL, O_WRONLY | O_NONBLOCK) != 0) {
          coral::throwSystemError("failed to put NotificationQueue pipe write "
                                  "endpoint into non-blocking mode", errno);
        }
      } catch (...) {
        ::close(pipeFds_[0]);
  ...
  bool tryConsume(MessageT& result) {
    ...
    try {
      ...
      coral::SpinLockGuard g(spinlock_);
 ...
 private:
  ...
  inline void signalEvent(size_t numAdded = 1) const {
    ...
    if (bytes_written != bytes_expected) {
      coral::throwSystemError("failed to signal NotificationQueue after "
                              "write", errno);
    }
  }
  ...
  mutable coral::SpinLock spinlock_;
  int eventfd_;
...
void NotificationQueue<MessageT>::Consumer::init(
  ...
  {
    coral::SpinLockGuard g(queue_->spinlock_);
