...
class AsyncUDPServerSocket : private AsyncUDPSocket::ReadCallback
 ...
 public:
  ...
  void bind(const coral::SocketAddress& addy) {
    CHECK(!socket_);
 ...
 private:
  ...
  void onDataAvailable(const coral::SocketAddress& clientAddress,
                       size_t len,
    ...
    auto mvp =
        coral::MoveWrapper<
            std::unique_ptr<coral::IOBuf>>(std::move(data));
    auto socket = socket_;
