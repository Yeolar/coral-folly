...
#ifndef CORAL_IO_IOBUF_QUEUE_H
#define CORAL_IO_IOBUF_QUEUE_H
...
class IOBufQueue {
 public:
   ...
   */
  void append(std::unique_ptr<coral::IOBuf>&& buf,
              bool pack=false);
   ...
   *         in the queue.
   */
  std::unique_ptr<coral::IOBuf> split(size_t n);
   ...
   */
  std::unique_ptr<coral::IOBuf> move() {
    chainLength_ = 0;
   ...
   */
  std::unique_ptr<coral::IOBuf> pop_front();
 ...
 private:
  ...
  std::unique_ptr<coral::IOBuf> head_;
};

} // coral

#endif // CORAL_IO_IOBUF_QUEUE_H
