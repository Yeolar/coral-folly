...
ShutdownSocketSet::ShutdownSocketSet(size_t maxFd)
  : maxFd_(maxFd),
    data_(static_cast<std::atomic<uint8_t>*>(
            coral::checkedCalloc(maxFd, sizeof(std::atomic<uint8_t>)))),
...
retry:
  ...
  return newState == FREE ? coral::closeNoInt(fd) : 0;
}
...
void ShutdownSocketSet::shutdown(int fd, bool abortive) {
  ...
  coral::closeNoInt(fd);  // ignore errors, nothing to do
...
void ShutdownSocketSet::doShutdown(int fd, bool abortive) {
  ...
  coral::shutdownNoInt(fd, SHUT_RDWR);
  ...
  coral::dup2NoInt(nullFile_.fd(), fd);
}
