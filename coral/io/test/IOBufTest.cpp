...
using coral::fbvector;
using coral::IOBuf;
using coral::TypedIOBuf;
...
TEST(IOBuf, CopyConstructorAndAssignmentOperator) {
  ...
  {
    coral::IOBuf newBuf(coral::IOBuf::CREATE, 4096);
...
std::string toString(const coral::IOBuf& buf) {
  std::string result;
...
char* writableStr(coral::IOBuf& buf) {
