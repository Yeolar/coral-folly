...
#include <coral/io/Cursor-defs.h>
...
namespace {
void append(std::unique_ptr<IOBuf>& buf, coral::StringPiece data) {
...
TEST(IOBuf, Gather) {
  ...
  EXPECT_EQ("lo world",
            coral::StringPiece(reinterpret_cast<const char*>(cursor.data()),
                               cursor.length()));
...
TEST(IOBuf, cloneAndInsert) {
...

  {
    ...
    cursor.insert(coral::IOBuf::create(0));
