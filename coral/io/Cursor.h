...
#ifndef CORAL_CURSOR_H
#define CORAL_CURSOR_H
...
class CursorBase {
 ...
 public:
  ...
  void clone(std::unique_ptr<coral::IOBuf>& buf, size_t len) {
    if (UNLIKELY(cloneAtMost(buf, len) != len)) {
      throw std::out_of_range("underflow");
    }
  }

  void clone(coral::IOBuf& buf, size_t len) {
    if (UNLIKELY(cloneAtMost(buf, len) != len)) {
      throw std::out_of_range("underflow");
    }
  }

  size_t cloneAtMost(coral::IOBuf& buf, size_t len) {
    buf = coral::IOBuf();

    std::unique_ptr<coral::IOBuf> tmp;
    size_t copied = 0;
  ...
  size_t cloneAtMost(std::unique_ptr<coral::IOBuf>& buf, size_t len) {
    if (!buf) {
      buf = make_unique<coral::IOBuf>();
    }
...
class RWCursor
 ...
 public:
  ...
  void insert(std::unique_ptr<coral::IOBuf> buf) {
    coral::IOBuf* nextBuf;
    if (this->offset_ == 0) {
      // Can just prepend
      ...
      std::unique_ptr<coral::IOBuf> remaining;
...
class Appender : public detail::Writable<Appender> {
 public:
   ...
   * format specifier.
   *
   * Note that coral/Format.h provides nicer and more type-safe mechanisms
   ...
   */
  void printf(CORAL_PRINTF_FORMAT const char* fmt, ...)
...
class QueueAppender : public detail::Writable<QueueAppender> {
 ...
 private:
  coral::IOBufQueue* queue_;
  size_t growth_;
};

}}  // coral::io

#endif // CORAL_CURSOR_H
