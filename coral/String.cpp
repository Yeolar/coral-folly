...
double prettyToDouble(coral::StringPiece *const prettyString,
  ...
  double value = coral::to<double>(prettyString);
  ...
  if (bestPrefixId == -1) { //No valid suffix rule found
    throw std::invalid_argument(coral::to<std::string>(
...
#elif defined(CORAL_HAVE_XSI_STRERROR_R) || \
...
#ifdef CORAL_DEFINED_DMGL
# undef CORAL_DEFINED_DMGL
# undef DMGL_NO_OPTS
# undef DMGL_PARAMS
# undef DMGL_ANSI
# undef DMGL_JAVA
