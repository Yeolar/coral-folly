...
# define CORAL_SYNCHRONIZED_HAVE_TIMED_MUTEXES 0
#else
# define CORAL_SYNCHRONIZED_HAVE_TIMED_MUTEXES 1
#endif
...
#if CORAL_SYNCHRONIZED_HAVE_TIMED_MUTEXES
...
#endif // CORAL_SYNCHRONIZED_HAVE_TIMED_MUTEXES
 ...
 * boost::recursive_timed_mutex, and the coral/RWSpinLock.h
 * classes.
 *
 ...
 *
 * Refer to coral/docs/Synchronized.md for a detailed explanation and more
 * examples.
 */
#define SYNCHRONIZED(...)                                       \
  if (bool SYNCHRONIZED_state = false) {} else                  \
    for (auto SYNCHRONIZED_lockedPtr =                          \
           (CR_ARG_2_OR_1(__VA_ARGS__)).operator->();           \
         !SYNCHRONIZED_state; SYNCHRONIZED_state = true)        \
      for (auto& CR_ARG_1(__VA_ARGS__) =                        \
...
#define TIMED_SYNCHRONIZED(timeout, ...)                           \
  if (bool SYNCHRONIZED_state = false) {} else                     \
    for (auto SYNCHRONIZED_lockedPtr =                             \
           (CR_ARG_2_OR_1(__VA_ARGS__)).timedAcquire(timeout);     \
         !SYNCHRONIZED_state; SYNCHRONIZED_state = true)           \
      for (auto CR_ARG_1(__VA_ARGS__) =                            \
             SYNCHRONIZED_lockedPtr.operator->();                  \
           !SYNCHRONIZED_state; SYNCHRONIZED_state = true)
...
#define SYNCHRONIZED_CONST(...)                         \
  SYNCHRONIZED(CR_ARG_1(__VA_ARGS__),                   \
               (CR_ARG_2_OR_1(__VA_ARGS__)).asConst())

/**
 * Similar to TIMED_SYNCHRONIZED, but only uses a read lock.
 */
#define TIMED_SYNCHRONIZED_CONST(timeout, ...)                  \
  TIMED_SYNCHRONIZED(timeout, CR_ARG_1(__VA_ARGS__),            \
                     (CR_ARG_2_OR_1(__VA_ARGS__)).asConst())
