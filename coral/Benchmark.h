...
#define BENCHMARK_IMPL(funName, stringName, rv, paramType, paramName)   \
  ...
  static bool CR_ANONYMOUS_VARIABLE(coralBenchmarkUnused) = (           \
    ::coral::addBenchmark(__FILE__, stringName,                         \
...
#define BENCHMARK(name, ...)                                    \
  BENCHMARK_IMPL(                                               \
    name,                                                       \
    CR_STRINGIZE(name),                                         \
    CR_ARG_2_OR_1(1, ## __VA_ARGS__),                           \
    CR_ONE_OR_NONE(unsigned, ## __VA_ARGS__),                   \
    __VA_ARGS__)
...
#define BENCHMARK_NAMED_PARAM(name, param_name, ...)                    \
  BENCHMARK_IMPL(                                                       \
      CR_CONCATENATE(name, CR_CONCATENATE(_, param_name)),              \
      CR_STRINGIZE(name) "(" CR_STRINGIZE(param_name) ")",              \
      iters,                                                            \
      unsigned,                                                         \
      iters) {                                                          \
...
#define BENCHMARK_DRAW_LINE()                                             \
  static bool CR_ANONYMOUS_VARIABLE(coralBenchmarkUnused) = (             \
    ::coral::addBenchmark(__FILE__, "-", []() -> unsigned { return 0; }), \
    true);
...
#define BENCHMARK_SUSPEND                               \
  if (auto CR_ANONYMOUS_VARIABLE(BENCHMARK_SUSPEND) =   \
      ::coral::BenchmarkSuspender()) {}                 \
  else

#endif // CORAL_BENCHMARK_H_
