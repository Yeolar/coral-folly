...
#define CORAL_CACHE_LINE_SIZE 64

template <class T, size_t N>
struct CORAL_ALIGNED_MAX SpinLockArray {
  T& operator[](size_t i) {
    return data_[i].lock;
  }
 ...
 private:
  struct PaddedSpinLock {
    PaddedSpinLock() : lock() {}
    T lock;
    char padding[CORAL_CACHE_LINE_SIZE - sizeof(T)];
  };
  static_assert(sizeof(PaddedSpinLock) == CORAL_CACHE_LINE_SIZE,
                "Invalid size of PaddedSpinLock");

  // Check if T can theoretically cross a cache line.
  static_assert(alignof(std::max_align_t) > 0 &&
                CORAL_CACHE_LINE_SIZE % alignof(std::max_align_t) == 0 &&
                sizeof(T) <= alignof(std::max_align_t),
                "T can cross cache line boundaries");

  char padding_[CORAL_CACHE_LINE_SIZE];
