...
#ifndef CORAL_BASE_CONV_H_
#define CORAL_BASE_CONV_H_

#include <coral/FBString.h>
#include <coral/Likely.h>
#include <coral/Preprocessor.h>
...
#define CORAL_RANGE_CHECK_STRINGIZE(x) #x
#define CORAL_RANGE_CHECK_STRINGIZE2(x) CORAL_RANGE_CHECK_STRINGIZE(x)

// Android doesn't support std::to_string so just use a placeholder there.
#ifdef __ANDROID__
#define CORAL_RANGE_CHECK_TO_STRING(x) std::string("N/A")
#else
#define CORAL_RANGE_CHECK_TO_STRING(x) std::to_string(x)
#endif

#define CORAL_RANGE_CHECK(condition, message, src)                          \
  ((condition) ? (void)0 : throw std::range_error(                          \
    (std::string(__FILE__ "(" CORAL_RANGE_CHECK_STRINGIZE2(__LINE__) "): ") \
     + (message) + ": '" + (src) + "'").c_str()))

#define CORAL_RANGE_CHECK_BEGIN_END(condition, message, b, e)    \
  CORAL_RANGE_CHECK(condition, message, std::string((b), (e) - (b)))

#define CORAL_RANGE_CHECK_STRINGPIECE(condition, message, sp)    \
  CORAL_RANGE_CHECK(condition, message, std::string((sp).data(), (sp).size()))
...
to(const Src & value) {
  /* static */ if (std::numeric_limits<Tgt>::max()
    ...
    CORAL_RANGE_CHECK(
      ...
      "Overflow",
      CORAL_RANGE_CHECK_TO_STRING(value));
  }
...
to(const Src & value) {
  /* static */ if (std::numeric_limits<Tgt>::max() <
                   std::numeric_limits<Src>::max()) {
    CORAL_RANGE_CHECK(value <= std::numeric_limits<Tgt>::max(),
                      "Overflow",
                      CORAL_RANGE_CHECK_TO_STRING(value));
    CORAL_RANGE_CHECK(value >= -std::numeric_limits<Tgt>::max(),
                      "Negative overflow",
...
inline uint32_t digits10(uint64_t v) {
  ...
  static const uint64_t powersOf10[20] CORAL_ALIGNED(64) = {
...
estimateSpaceNeeded(Src value) {
  const char *c = value;
  if (c) {
    return coral::StringPiece(value).size();
  };
  return 0;
}
...
template<class Src>
typename std::enable_if<
  (std::is_convertible<Src, coral::StringPiece>::value ||
...
toAppend(const Ts&... vs) {
  ::coral::detail::toAppendStrImpl(vs...);
}
...
toAppendFit(const Ts&... vs) {
  ::coral::detail::reserveInTarget(vs...);
  toAppend(vs...);
}
...
CORAL_ALIGNED(16) constexpr uint16_t shift1[] = {
...
CORAL_ALIGNED(16) constexpr uint16_t shift10[] = {
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  // 0-9
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  10
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  20
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  30
...
CORAL_ALIGNED(16) constexpr uint16_t shift100[] = {
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  // 0-9
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  10
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  20
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  30
...
CORAL_ALIGNED(16) constexpr uint16_t shift1000[] = {
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  // 0-9
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  10
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  20
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  30
 ...
 */
  ...
  Tgt digits_to(const char * b, const char * e) {
    ...
    assert(b == e);
    CORAL_RANGE_CHECK_BEGIN_END(size > 0,
...
typename std::enable_if<
  ...
  Tgt>::type
to(const char * b, const char * e) {
  CORAL_RANGE_CHECK(b < e, "Empty input string in conversion to integral",
                    to<std::string>("b: ", intptr_t(b), " e: ", intptr_t(e)));
  if (!isdigit(*b)) {
    if (*b == '-') {
      Tgt result = -to<typename std::make_unsigned<Tgt>::type>(b + 1, e);
      CORAL_RANGE_CHECK_BEGIN_END(result <= 0, "Negative overflow.", b, e);
      return result;
    }
    CORAL_RANGE_CHECK_BEGIN_END(*b == '+', "Invalid lead character", b, e);
    ++b;
  }
  Tgt result = to<typename std::make_unsigned<Tgt>::type>(b, e);
  CORAL_RANGE_CHECK_BEGIN_END(result >= 0, "Overflow", b, e);
  return result;
}
...
to(StringPiece * src) {
  ...
  for (;; ++b) {
    CORAL_RANGE_CHECK_STRINGPIECE(b < past,
  ...
  /* static */ if (std::is_signed<Tgt>::value) {
    if (!isdigit(*m)) {
      if (*m == '-') {
        ...
        CORAL_RANGE_CHECK_STRINGPIECE(*m == '+', "Invalid leading character in "
  ...
  CORAL_RANGE_CHECK_STRINGPIECE(m < past, "No digits found in input string",
                                *src);
  CORAL_RANGE_CHECK_STRINGPIECE(isdigit(*m), "Non-digit character found", *src);
  ...
  /* static */ if (!std::is_signed<Tgt>::value) {
    ...
    if (negative) {
      result = -t;
      CORAL_RANGE_CHECK_STRINGPIECE(is_non_positive(result),
                                    "Negative overflow", *src);
    } else {
      result = t;
      CORAL_RANGE_CHECK_STRINGPIECE(is_non_negative(result), "Overflow", *src);
    }
  }
...
inline void enforceWhitespace(const char* b, const char* e) {
  for (; b != e; ++b) {
    CORAL_RANGE_CHECK_BEGIN_END(isspace(*b),
...
to(StringPiece *const src) {
  ...
  CORAL_RANGE_CHECK_STRINGPIECE(!src->empty(),
                                "No digits found in input string", *src);
...
// CORAL_CONV_INTERNAL is defined by Conv.cpp.  Keep the CORAL_RANGE_CHECK
...
#ifndef CORAL_CONV_INTERNAL
#undef CORAL_RANGE_CHECK
#undef CORAL_RANGE_CHECK_BEGIN_END
#undef CORAL_RANGE_CHECK_STRINGPIECE
#undef CORAL_RANGE_CHECK_STRINGIZE
#undef CORAL_RANGE_CHECK_STRINGIZE2
#endif

#endif /* CORAL_BASE_CONV_H_ */
