...
#ifndef CORAL_BASE_FBSTRING_H_
#define CORAL_BASE_FBSTRING_H_
...
// Handle the cases where the fbcode version (coral/Malloc.h) is included
// either before or after this inclusion.
#ifdef CORAL_MALLOC_H_
#undef CORAL_MALLOC_H_
...
#if CORAL_HAVE_DEPRECATED_ASSOC
...
// use std::__throw_*
// nolint
#define throw CORAL_FBSTRING_MAY_NOT_USE_THROW
...
CORAL_ASSUME_RELOCATABLE(basic_fbstring<T, R, A, S>);
...
#define CORAL_FBSTRING_HASH1(T) \
  template <> \
  struct hash< ::coral::basic_fbstring<T> > { \
    size_t operator()(const ::coral::fbstring& s) const { \
      return ::coral::hash::fnv32_buf(s.data(), s.size()); \
    } \
  };

// The C++11 standard says that these four are defined
#define CORAL_FBSTRING_HASH \
  CORAL_FBSTRING_HASH1(char) \
  CORAL_FBSTRING_HASH1(char16_t) \
  CORAL_FBSTRING_HASH1(char32_t) \
  CORAL_FBSTRING_HASH1(wchar_t)
...
#endif // CORAL_HAVE_DEPRECATED_ASSOC

#undef CORAL_FBSTRING_HASH
#undef CORAL_FBSTRING_HASH1
