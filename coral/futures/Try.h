 ...
 public:
  ...
  explicit Try(exception_wrapper e)
    : contains_(Contains::EXCEPTION),
      e_(coral::make_unique<exception_wrapper>(std::move(e))) {}
   ...
   */
  CORAL_DEPRECATED("use Try(exception_wrapper)")
  explicit Try(std::exception_ptr ep)
    : contains_(Contains::EXCEPTION) {
    try {
      std::rethrow_exception(ep);
    } catch (const std::exception& e) {
      e_ = coral::make_unique<exception_wrapper>(std::current_exception(), e);
    } catch (...) {
      e_ = coral::make_unique<exception_wrapper>(std::current_exception());
    }
  }
...
class Try<void> {
 public:
  ...
  Try& operator=(const Try<void>& t) {
    ...
    if (t.e_) {
      e_ = coral::make_unique<exception_wrapper>(*t.e_);
    }
    return *this;
  }
  // Copy constructor
...
#include <coral/futures/Try-inl.h>
