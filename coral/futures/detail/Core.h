...
class Core {
 ...
 public:
  ...
  void raise(exception_wrapper e) {
    ...
    if (!interrupt_ && !hasResult()) {
      interrupt_ = coral::make_unique<exception_wrapper>(std::move(e));
 ...
 protected:
  ...
  coral::Optional<Try<T>> result_;
  std::function<void(Try<T>&&)> callback_ {nullptr};
  FSM<State> fsm_;
  std::atomic<unsigned char> attached_;
  std::atomic<bool> active_ {true};
  std::atomic<bool> interruptHandlerSet_ {false};
  coral::MicroSpinLock interruptLock_ {0};
  coral::MicroSpinLock executorLock_ {0};
  int8_t priority_ {-1};
