...
#include <coral/futures/FutureException.h>
#include <coral/futures/detail/Types.h>

// boring predeclarations and details
#include <coral/futures/Future-pre.h>

// not-boring helpers, e.g. all in coral::futures, makeFuture variants, etc.
// Needs to be included after Future-pre.h and before Future-inl.h
#include <coral/futures/helpers.h>
...
class Future {
 public:
  ...
  CORAL_DEPRECATED("do not use") Future<T>& activate() & {
    core_->activate();
    return *this;
  }
  CORAL_DEPRECATED("do not use") Future<T>& deactivate() & {
    core_->deactivate();
    return *this;
  }
  CORAL_DEPRECATED("do not use") Future<T> activate() && {
  ...
  /// is a coral::PredicateDoesNotObtain exception
  template <class F>
...
#include <coral/futures/Future-inl.h>
