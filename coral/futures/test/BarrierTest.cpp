...
#include <coral/futures/Barrier.h>
...
namespace coral { namespace futures { namespace test {
...
TEST(BarrierTest, Random) {
  ...
  auto numThreads = coral::Random::rand32(30, 91);
  ...
  for (auto& tinfo : threads) {
    tinfo.numFutures = coral::Random::rand32(100);
  ...
  for (auto& tinfo : threads) {
    ...
    tinfo.thread = std::thread(
        [numIterations, pinfo, &barrier] () {
          std::vector<coral::Future<bool>> futures;
          ...
          for (uint32_t i = 0; i < numIterations; ++i, ++pinfo->iteration) {
            ...
            for (uint32_t j = 0; j < pinfo->numFutures; ++j) {
              ...
              auto nanos = coral::Random::rand32(10 * 1000 * 1000);
              /* sleep override */
              std::this_thread::sleep_for(std::chrono::nanoseconds(nanos));
            }
            auto results = coral::collect(futures).get();
            pinfo->trueSeen[i] =
