...
TEST(Future, detachRace) {
  ...
  auto p = coral::make_unique<Promise<bool>>();
  auto f = coral::make_unique<Future<bool>>(p->getFuture());
  coral::Baton<> baton;
  std::thread t1([&]{
    baton.post();
    p.reset();
...
TEST(Future, CircularDependencySharedPtrSelfReset) {
  ...
  ptr->then(
    [ptr] (coral::Try<int64_t>&& uid) mutable {
...
TEST(Future, thenDynamic) {
  // coral::dynamic has a constructor that takes any T, this test makes
  // sure that we call the then lambda with coral::dynamic and not
  // Try<coral::dynamic> because that then fails to compile
  Promise<coral::dynamic> p;
  Future<coral::dynamic> f = p.getFuture().then(
      [](const coral::dynamic& d) {
        return coral::dynamic(d.asInt() + 3);
      }
  );
  p.setValue(2);
  EXPECT_EQ(f.get(), 5);
...
TEST(Future, RequestContext) {
  ...
  RequestContext::get()->setContextData("key",
      coral::make_unique<MyRequestData>(true));
