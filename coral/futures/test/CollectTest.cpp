...
auto rng = std::mt19937(coral::randomNumberSeed());
...
/// Ensure that we can compile collectAll/Any with coral::small_vector
TEST(Collect, smallVector) {
  static_assert(!CORAL_IS_TRIVIALLY_COPYABLE(Future<Unit>),
                "Futures should not be trivially copyable");
  static_assert(!CORAL_IS_TRIVIALLY_COPYABLE(Future<int>),
                "Futures should not be trivially copyable");

  {
    coral::small_vector<Future<Unit>> futures;
