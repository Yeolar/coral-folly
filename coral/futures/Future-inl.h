...
#include <coral/futures/Timekeeper.h>
...
Future<T>::thenImplementation(F func, detail::argResult<isTry, F, Args...>) {
  ...
  coral::MoveWrapper<Promise<B>> p;
  p->core_->setInterruptHandlerNoLock(core_->getInterruptHandler());
  coral::MoveWrapper<F> funcm(std::forward<F>(func));
...
Future<T>::onError(F&& func) {
  ...
  p.core_->setInterruptHandlerNoLock(core_->getInterruptHandler());
  auto f = p.getFuture();
  auto pm = coral::makeMoveWrapper(std::move(p));
  auto funcm = coral::makeMoveWrapper(std::move(func));
...
Future<T> Future<T>::onTimeout(Duration dur, F&& func, Timekeeper* tk) {
  auto funcw = coral::makeMoveWrapper(std::forward<F>(func));
...
Future<T> reduce(It first, It last, T&& initial, F&& func) {
  ...
  coral::MoveWrapper<T> minitial(std::move(initial));
...
Future<I> Future<T>::reduce(I&& initial, F&& func) {
  coral::MoveWrapper<I> minitial(std::move(initial));
  coral::MoveWrapper<F> mfunc(std::move(func));
...
Future<T> unorderedReduce(It first, It last, T initial, F func) {
  ...
  struct UnorderedReduceContext {
    UnorderedReduceContext(T&& memo, F&& fn, size_t n)
      ...
      {};
    coral::MicroSpinLock lock_; // protects memo_ and numThens_
    Future<T> memo_;
    F func_;
  ...
  mapSetCallback<ItT>(first, last, [ctx](size_t i, Try<ItT>&& t) {
    coral::MoveWrapper<Try<ItT>> mt(std::move(t));
...
Future<T> Future<T>::within(Duration dur, E e, Timekeeper* tk) {
  ...
  if (!tk) {
    tk = coral::detail::getTimekeeperSingleton();
  }
...
void waitImpl(Future<T>& f, Duration dur) {
  ...
  auto baton = std::make_shared<coral::fibers::Baton>();
...
Future<T> Future<T>::filter(F predicate) {
  auto p = coral::makeMoveWrapper(std::move(predicate));
...
Future<Unit> times(const int n, F thunk) {
  auto count = coral::makeMoveWrapper(
    std::unique_ptr<std::atomic<int>>(new std::atomic<int>(0))
  );
  return coral::whileDo([=]() mutable {
...
struct retrying_policy_traits {
  ...
  CORAL_CREATE_HAS_MEMBER_FN_TRAITS(has_op_call, operator());
  template <class Ret>
