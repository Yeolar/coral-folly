...
void MemoryIdler::flushLocalMallocCaches() {
  if (usingJEMalloc()) {
    if (!mallctl) {
      CR_LOG_EVERY_MS(ERROR, 10000) << "mallctl weak link failed";
      return;
    }
...
#if CORAL_X64 && defined(_GNU_SOURCE) && defined(__linux__)

static const size_t s_pageSize = sysconf(_SC_PAGESIZE);
static CORAL_TLS uintptr_t tls_stackLimit;
static CORAL_TLS size_t tls_stackSize;
...
static void fetchStackLimits() {
  ...
  if ((err = pthread_attr_getstack(&attr, &addr, &rawSize))) {
    ...
    CR_LOG_EVERY_MS(ERROR, 10000) << "pthread_attr_getstack error " << err;
    assert(false);
    tls_stackSize = 1;
    return;
  }
...
CORAL_NOINLINE static uintptr_t getStackPtr() {
  char marker;
