...
inline std::string familyNameStr(sa_family_t family) {
  switch (family) {
    ...
    case AF_UNIX:
      return "AF_UNIX";
    default:
      return coral::format("sa_family_t({})",
          coral::to<std::string>(family)).str();
  }
}
...
inline bool getNthMSBitImpl(const IPAddrType& ip, uint8_t bitIndex,
  ...
  if (bitIndex >= ip.bitCount()) {
    throw std::invalid_argument(coral::to<std::string>("Bit index must be < ",
...
struct Bytes : private boost::noncopyable {
  ...
  longestCommonPrefix(
    ...
    if (oneMask > kBitCount || twoMask > kBitCount) {
      throw std::invalid_argument(coral::to<std::string>("Invalid mask "
