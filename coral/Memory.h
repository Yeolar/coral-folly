...
#ifndef CORAL_MEMORY_H_
#define CORAL_MEMORY_H_
 ...
 *
 *      using BIO_deleter = coral::static_function_deleter<BIO, &BIO_free>;
...
class is_simple_allocator {
  CORAL_CREATE_HAS_MEMBER_FN_TRAITS(has_destroy, destroy);
...
struct AllocatorUniquePtr {
  typedef std::unique_ptr<T,
    coral::allocator_delete<
      typename std::conditional<
        is_simple_allocator<T, Allocator>::value,
        coral::StlAllocator<typename std::remove_reference<Allocator>::type, T>,
        typename std::remove_reference<Allocator>::type
      >::type
    >
  > type;
...
typename AllocatorUniquePtr<T, Allocator>::type allocate_unique(
  ...
  auto stlAllocator = coral::make_stl_allocator<T>(
  ...
  try {
    ...
    return {p,
      coral::allocator_delete<decltype(stlAllocator)>(std::move(stlAllocator))
    };
  } catch (...) {
...
std::shared_ptr<T> allocate_shared(Allocator&& allocator, Args&&... args) {
  return std::allocate_shared<T>(
    coral::make_stl_allocator<T>(std::forward<Allocator>(allocator)),
