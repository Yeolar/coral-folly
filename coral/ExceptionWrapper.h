...
#ifndef CORAL_EXCEPTIONWRAPPER_H
#define CORAL_EXCEPTIONWRAPPER_H

#include <cassert>
#include <exception>
#include <memory>
#include <coral/String.h>
#include <coral/detail/ExceptionWrapper.h>
 ...
 * that prefer rethrowing the exception. We could use a coral::dynamic for every
...
class exception_wrapper {
 ...
 public:
  ...
  /* implicit */ exception_wrapper(Ex&& exn) {
    ...
    throwfn_ = coral::detail::Thrower<DEx>::doThrow;
  }
...
exception_wrapper make_exception_wrapper(Args&&... args) {
  ...
  ew.throwfn_ = coral::detail::Thrower<T>::doThrow;
  return ew;
}
 ...
 * auto ew = coral::try_and_catch<std::exception, std::runtime_error>([=]() {
 *   if (badThingHappens()) {
 ...
 * auto ew = coral::try_and_catch<std::runtime_error>([=]() {
 *   if (badThingHappens()) {
 *     throw std::exception();
 *   }
 * });
...
class try_and_catch<LastException, Exceptions...> :
 ...
 protected:
  ...
  assign_exception(Ex& e, std::exception_ptr /*eptr*/) {
    ...
    this->throwfn_ = coral::detail::Thrower<Ex>::doThrow;
  }
