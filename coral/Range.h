...
#ifndef CORAL_RANGE_H_
#define CORAL_RANGE_H_
...
public:
  ...
  size_type rfind(value_type c) const {
    return coral::rfind(castToConst(), c);
  }
   ...
   * Example:
   *
   *  coral::StringPiece s("sample string for split_next");
   *  auto p = s.split_step(' ');
   *
   ...
   * Example:
   *
   *  void do_some_parsing(coral::StringPiece s) {
   *    auto version = s.split_step(' ', [&](coral::StringPiece x) {
   *      if (x.empty()) {
   ...
   *
   *  struct Foo {
   *    void parse(coral::StringPiece s) {
   *      s.split_step(' ', parse_field, bar, 10);
   *      s.split_step('\t', parse_field, baz, 20);
   *
   *      auto const kludge = [](coral::StringPiece x, int &out, int def) {
   *        if (x == "null") {
   *          out = 0;
   *        } else {
   ...
   *    int foo;
   *
   *    static parse_field(coral::StringPiece s, int &out, int def) {
   *      try {
   *        out = coral::to<int>(s);
...
#if CORAL_HAVE_EMMINTRIN_H && __GNUC_PREREQ(4, 6)
...
inline size_t qfind_first_byte_of(const StringPiece haystack,
  ...
  static auto const qfind_first_byte_of_fn =
    coral::CpuId().sse42() ? qfind_first_byte_of_sse42
...
template <class T>
struct hasher<coral::Range<T*>,
              typename std::enable_if<std::is_pod<T>::value, void>::type> {
  size_t operator()(coral::Range<T*> r) const {
...
CORAL_ASSUME_FBVECTOR_COMPATIBLE_1(coral::Range);

#endif // CORAL_RANGE_H_
