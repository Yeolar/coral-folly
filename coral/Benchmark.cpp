...
#define CR_CORAL_GLOBAL_BENCHMARK_BASELINE crCoralGlobalBenchmarkBaseline
#define CR_STRINGIZE_X2(x) CR_STRINGIZE(x)

// Add the global baseline
BENCHMARK(CR_CORAL_GLOBAL_BENCHMARK_BASELINE) {
#ifdef _MSC_VER
  _ReadWriteBarrier();
#else
  asm volatile("");
...
int getGlobalBenchmarkBaselineIndex() {
  const char *global = CR_STRINGIZE_X2(CR_CORAL_GLOBAL_BENCHMARK_BASELINE);
  auto it = std::find_if(
    benchmarks().begin(),
    benchmarks().end(),
...
#undef CR_STRINGIZE_X2
#undef CR_CORAL_GLOBAL_BENCHMARK_BASELINE
