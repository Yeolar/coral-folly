...
char32_t utf8ToCodePoint(const unsigned char*& p,
  ...
  if (p >= e) {
    ...
    throw std::runtime_error("coral::utf8ToCodePoint empty/invalid string");
  }
  ...
  if ((fst & 0xC0) != 0xC0) {
    ...
    throw std::runtime_error(
      to<std::string>("coral::utf8ToCodePoint i=0 d=", d));
  }
  ...
  for (unsigned int i = 1; i != 3 && p + i < e; ++i) {
    ...
    if ((tmp & 0xC0) != 0x80) {
      if (skipOnError) return skip();
      throw std::runtime_error(
        to<std::string>("coral::utf8ToCodePoint i=", i,
                        " tmp=", (uint32_t)tmp));
    }
    ...
    if (!(fst & 0x80)) {
      ...
      if ((d & ~bitMask[i - 1]) == 0) {
        if (skipOnError) return skip();
        throw std::runtime_error(
          to<std::string>("coral::utf8ToCodePoint i=", i, " d=", d));
      }
  ...
  if (skipOnError) return skip();
  throw std::runtime_error("coral::utf8ToCodePoint encoding length maxed out");
}
...
std::string toLower(const coral::StringPiece& sp) {
  std::string result;
...
std::string toUpper(const coral::StringPiece& sp) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
...
std::string toOppositeCase(const coral::StringPiece& sp) {
  std::string result;
...
std::string normalize(const coral::StringPiece& sp, NormalizationMode mode) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
...
std::string asciify(const coral::StringPiece& sp) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
  icu::UnicodeString us = icu::UnicodeString::fromUTF8(usp);
  UErrorCode err = U_ZERO_ERROR;
