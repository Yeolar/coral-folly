...
void SocketAddress::setFromSockaddr(const struct sockaddr* address) {
  ...
  setFromIpAddrPort(coral::IPAddress(address), port);
}
...
const coral::IPAddress& SocketAddress::getIPAddress() const {
  auto family = getFamily();
  if (family != AF_INET && family != AF_INET6) {
...
bool SocketAddress::tryConvertToIPv4() {
  ...
  storage_.addr = coral::IPAddress::createIPv4(storage_.addr);
  return true;
}
...
bool SocketAddress::mapToIPv6() {
  ...
  storage_.addr = coral::IPAddress::createIPv6(storage_.addr);
  return true;
}
...
bool SocketAddress::prefixMatch(const SocketAddress& other,
  ...
  switch (getFamily()) {
    ...
    case AF_INET6:
    {
      auto prefix = coral::IPAddress::longestCommonPrefix(
...
size_t SocketAddress::hash() const {
  size_t seed = coral::hash::twang_mix64(getFamily());
  ...
  if (external_) {
    ...
    for (unsigned int n = 0; n < pathLength; ++n) {
      boost::hash_combine(seed, coral::hash::twang_mix64(path[n]));
    }
  }
...
void SocketAddress::setFromSocket(SocketDesc socket, GetPeerNameFunc fn) {
  ...
  if (fn(socket, (sockaddr*)&tmp_sock, &addrLen) != 0) {
    coral::throwSystemError("setFromSocket() failed");
  }
