...
#include <coral/IPAddressException.h>
#include <coral/IPAddressV4.h>
#include <coral/IPAddressV6.h>
#include <coral/detail/IPAddress.h>
...
class IPAddress : boost::totally_ordered<IPAddress> {
 public:
  ...
  static std::string networkToString(const CIDRNetwork& network) {
    return network.first.str() + "/" + coral::to<std::string>(network.second);
  }
...
// Define toAppend() to allow IPAddress to be used with coral::to<string>
...
namespace std {
template<>
struct hash<coral::IPAddress> {
  size_t operator()(const coral::IPAddress& addr) const {
    return addr.hash();
  }
};
}  // std
