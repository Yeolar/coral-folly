...
#ifndef CORAL_BITS_H_
#define CORAL_BITS_H_

#if !defined(__clang__) && !defined(_MSC_VER)
#define CORAL_INTRINSIC_CONSTEXPR constexpr
#else
// GCC is the only compiler with intrinsics constexpr.
#define CORAL_INTRINSIC_CONSTEXPR const
#endif

#include <coral/Portability.h>

#include <coral/detail/BitsDetail.h>
#include <coral/detail/BitIteratorDetail.h>
#include <coral/Likely.h>

#if CORAL_HAVE_BYTESWAP_H
# include <byteswap.h>
#endif
...
template <class T>
inline CORAL_INTRINSIC_CONSTEXPR
...
CR_GEN(uint8_t,)
#ifdef _MSC_VER
CR_GEN( int64_t, _byteswap_uint64)
CR_GEN(uint64_t, _byteswap_uint64)
CR_GEN( int32_t, _byteswap_ulong)
CR_GEN(uint32_t, _byteswap_ulong)
CR_GEN( int16_t, _byteswap_ushort)
CR_GEN(uint16_t, _byteswap_ushort)
#else
CR_GEN( int64_t, __builtin_bswap64)
CR_GEN(uint64_t, __builtin_bswap64)
CR_GEN( int32_t, __builtin_bswap32)
CR_GEN(uint32_t, __builtin_bswap32)
CR_GEN( int16_t, our_bswap16)
CR_GEN(uint16_t, our_bswap16)
#endif
...
#define CR_GEN(sz) \
  CR_GEN2(uint##sz##_t, sz) \
  CR_GEN2(int##sz##_t, sz)
